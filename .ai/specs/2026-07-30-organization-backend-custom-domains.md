...
| Q2 | On a bound host, when a super-admin's `om_selected_tenant`/`om_selected_org` disagrees with the binding: hard-deny (403) or silently clamp? | Silently clamping a super-admin who believes they are looking at tenant X while seeing tenant Y is a data-integrity hazard on writes. The codebase already established the precedent: `selectionRejected` exists because *"writes MUST fail loudly on this so a record never lands under an org the caller did not intend"* (`organizationScope.ts:20-26`). | **Hard-deny (403)** with an explicit "switch host to change organization" message. |
...
host plumbed in. On a bound host, an override outside the binding is a **403** (Q2), not a quiet
rewrite.
