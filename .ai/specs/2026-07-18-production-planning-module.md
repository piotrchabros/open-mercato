# Production Planning Module (`@open-mercato/manufacturing`)

| Field | Value |
|-------|-------|
| **Status** | Implemented (phases 0–6; PRs #4278, #4279, #4283, #4284, #4288, pending merge) |
| **Created** | 2026-07-18 |
...
- Ships as a dedicated workspace package **`packages/manufacturing`** (module id `manufacturing`), enabled by one `apps/mercato/src/modules.ts` entry and gated by a **`manufacturing_enabled` feature toggle**. **Zero modifications to core modules** — all integration happens through events, widget injection, response enrichers, FK-id + snapshot, and `tryResolve` soft dependencies.
- The platform has **no warehouse, no purchasing, and no print-template engine** (verified 2026-07-18). This spec resolves those gaps explicitly: a minimal module-owned stock ledger behind a **`manufacturingStockProvider` DI seam** (decision *i*), MRP "buy" suggestions degraded to export + notification (decision *d*), and the QR/PDF shop traveler deferred (decision *f*).
...
The module is an **optional vertical**: platform installations that do not enable the `manufacturing_enabled` feature toggle see no UI, no routes, and no behavior change.
...
| b | Reason codes | **`dictionaries` module.** MVP seeds one org-scoped dictionary: `manufacturing-scrap-reasons` (idempotent upgrade action, pattern: customers `interaction-statuses`). `production-downtime-reasons` ships later with the OEE extension. | Tenant-configurable enumerations are a solved problem; planner already FK-references dictionary entries the same way. |
| c | MRP execution model | **Per-tenant asynchronous jobs.** An `MrpRun` row is created, then one queue job per tenant/org (worker `{ queue: 'manufacturing-mrp', id: 'production:mrp-run', concurrency: 1 }`) executes it with a `ProgressJob` for top-bar progress. Cyclic execution via a `scheduler` cron entry that enumerates tenants and enqueues one job each (fan-out; never one job iterating all tenants). | Matches the platform's queue+progress+scheduler stack; per-tenant fan-out prevents cross-tenant bleed and starvation. Runs are versioned audit objects per the draft. |
| d | MRP "buy" suggestions | **Degrade to export + notification.** `buy` suggestions can be acknowledged/dismissed and exported to CSV; an in-app notification targets users with `manufacturing.mrp.manage`. No purchase order is created (no purchasing module exists). `manufacturing.mrp_suggestion.accepted` is emitted so a future purchasing module can subscribe. | Honest degradation instead of a dead button; the event is the forward-compatible seam. |
| e | Operator authentication | **Standard login + dedicated minimal role.** ACL features `manufacturing.operator.view` / `manufacturing.operator.report` gate the operator surface; `setup.ts` seeds an "Operator" role holding only those two features. The operator page implements client-side inactivity auto-logout (default 15 min, org-configurable). PIN/badge login and offline kiosk mode are **platform-level backlog**, not module scope. | The platform has no PIN/card auth; improvising one inside a module would weaken security. A minimal-feature role bounds the shared-tablet blast radius. |
...
| Put the module in `packages/core/src/modules/manufacturing` | Manufacturing is not a core concern of every installation; core placement would drag entities/migrations into every deployment and contradicts the optional-vertical requirement. |
...
```
packages/manufacturing/                      # new workspace package @open-mercato/manufacturing
...
- Enabled via one entry in `apps/mercato/src/modules.ts`: `{ id: 'manufacturing', from: '@open-mercato/manufacturing' }` (same wiring as `scheduler`).
- Entire surface (menu items, routes, pages, subscribers, cron) is gated by the `manufacturing_enabled` feature toggle (module `feature_toggles`); toggle off ⇒ no observable change.
...
- **Widget**: a "Production" tab injected into spot `sales.document.detail.order:tabs` showing linked production orders and a "Create production order" action (visible only for products with `ProductPlanningParams.procurement = 'make'` and feature `manufacturing.orders.manage`).
...
- **Response enricher** on `sales:order`: `_production: { orders: [{ id, status, qtyPlanned, qtyCompleted }] } | null`, feature-gated by `manufacturing.orders.view`, batch `enrichMany`, fallback `null`. Never exposed on portal/customer-facing responses.

### MRP engine (performance-critical design)

1. **Bulk load per tenant/org** (a handful of scoped SQL queries, not per-entity ORM walks): active BOM + routing versions, planning params, on-hand + reservations (via `manufacturingStockProvider`), open production orders, demand (sales order lines with due dates via query engine, `tryResolve`d; plus min-stock/safety-stock deficits).
...
All tables: `id uuid PK`, `tenant_id`, `organization_id`, `created_at`, `updated_at`, `deleted_at` (soft delete) unless noted. Table prefix `manufacturing_`.

| Entity | Table | Key columns (beyond standard) |
|--------|-------|-------------------------------|
| `WorkCenter` | `manufacturing_work_centers` | `name`, `kind` (`machine\|manual\|line\|subcontractor`), `cost_rate_per_hour numeric`, `parallel_stations int`, `efficiency_factor numeric`, `availability_rule_set_id uuid null` (FK-id → planner, decision *a*), `is_active` |
| `ProductionBom` | `manufacturing_boms` | `product_id`, `variant_id null` (FK-id → catalog), `version int`, `status` (`draft\|active\|archived`), `valid_from/valid_to`, `name`; UNIQUE `(tenant_id, organization_id, product_id, variant_id, version)` |
| `ProductionBomItem` | `manufacturing_bom_items` | `bom_id`, `component_product_id`, `component_variant_id null`, `qty_per_unit numeric`, `uom varchar` (canonical unit code), `scrap_factor numeric default 0`, `is_phantom bool`, `operation_sequence int null` (material → operation assignment) |
| `Routing` | `manufacturing_routings` | `product_id`, `variant_id null`, `version int`, `status`, `name`; version pairs with BOM version = technology version |
| `RoutingOperation` | `manufacturing_routing_operations` | `routing_id`, `sequence int`, `name`, `work_center_id`, `setup_time_minutes`, `run_time_per_unit_seconds`, `is_reporting_point bool` (no alternatives in MVP) |
| `ProductPlanningParams` | `manufacturing_planning_params` | `product_id`, `variant_id null` UNIQUE per scope, `procurement` (`make\|buy`), `lead_time_days int`, `min_lot numeric`, `lot_multiple numeric`, `safety_stock numeric`, `backflush bool default true` |
| `StockItem` | `manufacturing_stock_items` | `product_id`, `variant_id null`, `uom`, `on_hand numeric`, `reserved numeric` (derived, maintained transactionally); UNIQUE per scope+product+variant |
| `StockBatch` | `manufacturing_stock_batches` | `stock_item_id`, `batch_number`, `on_hand numeric`, `expires_at null` (optional per product) |
| `StockMovement` | `manufacturing_stock_movements` | **append-only** (decision *h*): `movement_type` (`receipt\|issue\|adjustment`), `product_id`, `variant_id`, `batch_id null`, `qty numeric` (signed), `uom`, `reason_entry_id null` (dictionary), `source_type/source_id` (order, report, import, manual), `reverses_movement_id uuid null UNIQUE`; no `deleted_at` semantics — never deleted |
| `MaterialReservation` | `manufacturing_material_reservations` | `order_id`, `order_material_id`, `stock_item_id`, `batch_id null`, `qty numeric`, `uom`, `status` (`active\|released\|consumed`) |
| `ProductionOrder` | `manufacturing_orders` | `number` (sequence per org), `product_id`, `variant_id`, `qty_planned numeric`, `uom`, `due_date`, `priority int`, `status` (§ status machine), `source_type` (`sales_order\|mrp\|manual`), `source_id null`, `bom_version_id`, `routing_version_id`, `released_at`, `qty_completed`, `qty_scrapped` |
| `ProductionOrderOperation` | `manufacturing_order_operations` | snapshot rows (decision *g*): `order_id`, `sequence`, `name`, `work_center_id`, times, `is_reporting_point`, `status` (`pending\|in_progress\|done`), `qty_good`, `qty_scrap`, `source_operation_id` |
| `ProductionOrderMaterial` | `manufacturing_order_materials` | snapshot rows: `order_id`, `operation_sequence null`, `component_product_id/variant_id`, `qty_required`, `uom`, `scrap_factor`, `qty_issued`, `source_bom_item_id` |
| `ProductionReport` | `manufacturing_reports` | **append-only** + storno: `order_operation_id`, `reporter_user_id`, `qty_good`, `qty_scrap`, `scrap_reason_entry_id null`, `started_at/finished_at null`, `report_type` (`partial\|final`), `reverses_report_id null UNIQUE` |
| `MrpRun` | `manufacturing_mrp_runs` | `status` (`pending\|running\|completed\|failed`), `params jsonb`, `progress_job_id`, `started_at/finished_at`, `stats jsonb` |
| `MrpSuggestion` | `manufacturing_mrp_suggestions` | `run_id`, `suggestion_type` (`make\|buy\|reschedule\|cancel`), `product_id/variant_id`, `qty`, `uom`, `due_date`, `demand_source jsonb` (pegging), `status` (`open\|accepted\|dismissed\|superseded`), `carried_from_suggestion_id null` |
...
All routes under `/api/manufacturing/*`, `makeCrudRoute` with `indexer: { entityType }`, exported `openApi`, standard optimistic locking; writes go through commands.

| Resource | Routes | Notes |
|----------|--------|-------|
| Work centers | CRUD `/api/manufacturing/work-centers` | |
| BOMs | CRUD `/api/manufacturing/boms` (+ `GET …/[id]` detail with items, `POST …/[id]/copy-version`, `POST …/[id]/activate`, `GET …/[id]/cost-rollup`) | activate validates cycles (missing-components validation is a tracked follow-up); cost-rollup returns a **list-price-based estimate** with `priceBasis: 'catalog_list_price'` — catalog has no purchase/cost price kind, so a true acquisition-cost basis is deferred until a purchase-price source exists (see Deferred) |
| Routings | CRUD `/api/manufacturing/routings` (+ `GET …/[id]` detail with operations, copy-version, activate) | |
| Planning params | CRUD `/api/manufacturing/planning-params` | gated by `manufacturing.mrp.view/manage` (planning master data is the planista's domain; the technology features stay with technolog) |
| Stock | `GET /api/manufacturing/stock`, `GET /api/manufacturing/stock/batches`, `POST /api/manufacturing/stock/receipts`, `POST /api/manufacturing/stock/issues`, `POST /api/manufacturing/stock/adjustments`, `POST /api/manufacturing/stock/import` (CSV, streaming) | mutations are commands emitting movements; storno via `POST /api/manufacturing/stock/movements/[id]/reverse` |
| Orders | CRUD `/api/manufacturing/orders` (+ `POST …/[id]/release`, `POST …/[id]/cancel`, `GET …/[id]/shortages`) | sub-resources guarded by parent `updated_at` |
| Reports | `POST /api/manufacturing/reports` (+ `POST …/[id]/reverse`) | final report on last reporting point triggers FG receipt |
| Operator | `GET /api/manufacturing/operator/queue` | features `manufacturing.operator.*` only |
| MRP | `POST /api/manufacturing/mrp/runs`, `GET …/runs`, `GET …/runs/[id]/suggestions`, `POST …/suggestions/accept` (bulk), `POST …/suggestions/dismiss`, `GET …/suggestions/export` | accept(make) → draft order; accept(buy) → ack + event only (decision *d*) |

## Events

`createModuleEvents({ moduleId: 'production' })`, convention `module.entity.action`:

`manufacturing.work_center.{created,updated,deleted}`, `manufacturing.bom.{created,updated,deleted,activated}`, `manufacturing.routing.{created,updated,deleted,activated}`, `manufacturing.order.{created,updated,deleted,released,completed,cancelled}`, `manufacturing.report.created`, `manufacturing.report.reversed`, `manufacturing.stock_movement.created`, `manufacturing.mrp_run.completed` (clientBroadcast), `manufacturing.mrp_suggestion.accepted`.
...
|---------------|----------|---------------|
| Technology | `manufacturing.technology.view/manage` | admin, Technolog |
| Stock (mini-ledger) | `manufacturing.stock.view/manage` | admin, Planista, Magazynier-lite |
| Orders | `manufacturing.orders.view/manage` | admin, Planista |
| Reporting | `manufacturing.reports.view/manage` | admin, Planista, Kierownik |
| Operator surface | `manufacturing.operator.view/report` | admin, Operator (only these two — decision *e*) |
| MRP | `manufacturing.mrp.view/manage` | admin, Planista |
...
| Snapshot divergence bugs | Code reads live BOM instead of snapshot post-release | Medium | Orders | Decision *g*: downstream reads only `manufacturing_order_*` tables; P3 test asserts BOM edit post-release has no effect | — |
...
- **All contract surfaces are new** (module id, `manufacturing_*` tables, `/api/manufacturing/*` routes, `manufacturing.*` events, `manufacturing.*` ACL features, DI token `manufacturingStockProvider`, widget spot usage) — no existing surface changes, nothing to deprecate.
- New surfaces enter `BACKWARD_COMPATIBILITY.md` scope on first release: the **StockProvider interface, event ids, and ACL feature ids are ADDITIVE-ONLY** thereafter; `manufacturing_stock_*` tables are explicitly documented as internal (decision *i*) and excluded from the contract.
- Migrations are module-scoped (`packages/manufacturing/src/modules/manufacturing/migrations/` + snapshot); PRs ship migration files, no local `db:migrate` by agents.
...
- **2026-07-19 (rev 7, Phase 6 implemented — MVP complete)**: Quantity-based analytics shipped (late/at-risk orders; actual-vs-standard consumption on the exact backflush basis — per-operation cumulative good+scrap; scrap by reason with an unspecified bucket), every aggregate tenant+multi-org scoped with route-level assertions. Final sweep: search config indexes orders and BOMs only (operational/append-only records excluded), i18n usage audit clean, ACL audit confirms wildcard-aware matching throughout. Contract surfaces registered for BACKWARD_COMPATIBILITY on first release: module id `manufacturing`; 16 `manufacturing_*` tables; `/api/manufacturing/**` routes; 14 `manufacturing.*` event ids; 12 ACL feature ids; DI token `manufacturingStockProvider` (warehouse extraction seam); injection into `sales.document.detail.order:tabs`; toggle `manufacturing_enabled`; queue `manufacturing-mrp`; dictionary kind `manufacturing-scrap-reasons`; notification `manufacturing.mrp.buy_suggestion_accepted`; entity link manufacturing:work_center → planner rule set. Docs page `apps/docs/docs/framework/modules/manufacturing.mdx`. Spec remains in `.ai/specs/` root until the PR stack merges and deploys (lifecycle rule).
- **2026-07-19 (rev 6, Phase 5 implemented)**: Net MRP shipped per decisions (c)/(d): pure engine with composite productId::variantId identity, level-ordered netting, backward scheduling, lot sizing after cross-parent aggregation, order-stable pegging; MrpRun/MrpSuggestion persistence with order-stable carry-over (accepted/dismissed equivalents land superseded with carriedFromSuggestionId — zero re-emitted noise, reversed-input regression test); per-tenant worker with full ProgressJob lifecycle and idempotent retry; cron fan-out skips tenants with manufacturing_enabled off; accept: make → draft orders (sourceType mrp), buy → purchasing-seam event + feature-gated notification + formula-injection-safe CSV export. **Benchmark (KPI < 60 s): full 10k products / 5-level BOMs = ~72.5 ms; 1k smoke = ~2.9 ms** (engine compute; loaders separately query-count-bounded). Integration spec TC-PROD-009 drains the real queue worker in-process.
- **2026-07-18 (rev 5, Phase 4 implemented)**: Shop-floor reporting shipped: append-only ProductionReport with storno (reverses_report_id UNIQUE), backflush per planning-params flag (consumption = qtyPerUnit × (1+scrap) × (good+scrap), UoM-converted; unassigned materials consume on the last reporting point; provider insufficiencies collect as warnings), finished-goods receipt and order-quantity accrual happen ONLY on the last reporting-point final report. STATUS MACHINE DELTA: reversing that final report reopens the order completed→in_progress through a reversal-only guarded helper (not exposed as a user transition) so a corrected final report can be resubmitted — the public transition map stays unchanged. ACL: manufacturing.operator.report gates report creation (granted also to planista/kierownik — requireFeatures is AND-only, no OR mechanism exists); reports.manage gates reversal. manufacturing-scrap-reasons lives in the GENERIC dictionaries module (spec decision (b) parenthetical about the customers module-local pattern is superseded — the generic module was the row's primary decision and enables the shared manage UI). Operator panel per decisions (e)/(f) with client inactivity logout. Integration spec TC-PROD-008.
- **2026-07-18 (rev 4, Phase 3 implemented)**: Production order aggregate shipped: normative status machine (pure lib, exhaustively tested), release copies active BOM+routing as snapshot rows atomically and reserves materials per line up to free availability (shared components never over-reserved; provider race rejections reclassified into shortage lines so release always completes side effects), computed shortage list at release + on-demand GET (qty_required − qty_issued − active reservations vs free stock). Sales integration per spec: Production tab injected into sales.document.detail.order:tabs (feature+toggle gated), opt-in idempotent MTO subscriber (moduleConfigService key manufacturing.mto_auto_draft, default OFF, tryResolve-degraded), caller-feature-gated _production enricher on sales:sales_order (portal callers hold no manufacturing.orders.view — verified against the platform enricher runner). Orders UI with aggregate-lock actions. Integration specs TC-PROD-006/007.
- **2026-07-18 (rev 3, Phase 2 implemented)**: Stock ledger shipped per decisions (h)/(i)/(j): `ManufacturingStockProvider` DI seam (`manufacturingStockProvider`) with `StockLedgerService` default implementation; stock commands are `isUndoable: false` — storno (`manufacturing.stock.reverseMovement`, compensating movement with `reverses_movement_id`) IS the undo per decision (h), and commands stay on `commandBus` for audit; CSV import streams via sync_excel's `parseCsvDocumentBatches` with a 10k row cap whose response always reports the partial `{imported, failed, capExceeded, errors}` summary (retry safety); domain errors surface as translated `manufacturing.errors.*` keys. Note: core's optimistic-lock guard scans a curated module map that does not yet include `production` — adding it is a tracked follow-up.
- **2026-07-18 (rev 2, Phase 0+1 implemented)**: Toggle identifier fixed as `manufacturing_enabled`; role key `magazynier-lite`; planning-params ACL assigned to `manufacturing.mrp.view/manage`; BOM/routing single-record `GET …/[id]` detail routes added (multi-org scope via the module's `resolveOrganizationScopeFilter`, mirroring `makeCrudRoute`); BOM activate validates cycles (missing-components validation tracked as follow-up); cost rollup shipped as a **list-price-based estimate** (`priceBasis: 'catalog_list_price'`) because catalog exposes no purchase-price kind — purchase-price basis moved to Deferred. Integration specs TC-PROD-001..004.
- **2026-07-18**: Initial draft (rev 1). Platform-mapped rewrite of `open-mercato-modul-produkcja-spec.md` v0.1 with decisions (a)–(k) fixed after research + adversarial review panel: planner calendars reuse, dictionaries reason codes, per-tenant async MRP, degraded buy suggestions, operator auth without PIN, list-based reporting MVP, row-copy technology snapshot, append-only ledger with storno, `manufacturingStockProvider` seam, mini-ledger scope fence, per-phase test matrix.
