...
describe('Connect SLA administration API contracts', () => {
  ...
  it('passes backend route ids into edit forms and deletes through item routes', () => {
    ...
    expect(form).toContain('`connect-sla/${plural}/${encodeURIComponent(id)}`')
    expect(form).not.toContain('updatedAt: initialValues.updatedAt')
    expect(form.indexOf('const publication =')).toBeLessThan(form.indexOf('if (mode === "create")'))
    expect(form).toContain('router.replace(`${backHref}/${encodeURIComponent(id)}`)')
