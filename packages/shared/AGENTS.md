...
| `di/` | When setting up dependency injection (Awilix). Request containers run in `InjectionMode.CLASSIC`, which resolves factory dependencies by PARAMETER NAME and passes them positionally — so <!-- di-classic-proxy:allow-start -->`asFunction(({ em }) => …)` silently injects `undefined`, and `asFunction(({ em: entityManager }) => …)` / `asFunction((cradle) => …)` throw at resolution<!-- di-classic-proxy:allow-end -->. Use plain named parameters (`asFunction((em) => …)`) or chain `.proxy()`; the same applies to `asClass` constructors. Guards: `packages/core/src/__tests__/di-classic-proxy.test.ts` (lint) and `src/lib/di/__tests__/classic-injection-mode.test.ts` (semantics). The app-level hook (`src/di.ts` → `register`) is wired explicitly in BOTH bootstrap paths — `src/bootstrap.ts` for the Next.js runtime, `bootstrapFromAppRoot()` for worker/scheduler/CLI processes. Never rely on the legacy `import('@/di')` fallback: the alias does not exist outside the app bundler | `@open-mercato/shared/lib/di` |
