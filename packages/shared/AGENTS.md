...
| `di/` | When setting up dependency injection (Awilix). The app-level hook (`src/di.ts` → `register`) is wired explicitly in BOTH bootstrap paths — `src/bootstrap.ts` for the Next.js runtime, `bootstrapFromAppRoot()` for worker/scheduler/CLI processes. Never rely on the legacy `import('@/di')` fallback: the alias does not exist outside the app bundler. The request container uses `InjectionMode.CLASSIC`, so an `asFunction` factory MUST take one named parameter per DI key (a destructured or `cradle` parameter needs `.proxy()`) and MUST mark optional dependencies with a default value, never a TypeScript `?` (erased at transpile) — enforced by `packages/core/src/__tests__/di-classic-proxy.test.ts` | `@open-mercato/shared/lib/di` |
