 ...
 * backed by the module-owned `manufacturing_stock_*` tables. Only the
 * {@link ManufacturingStockProvider} interface + the `manufacturing.stock_movement.created`
 ...
 * `ManufacturingStockProvider` interface: a future implementation that instead
 ...
 */
export class StockLedgerService implements ManufacturingStockProvider {
  constructor(
  ...
  private static readonly movementIndexer: CrudIndexerConfig<StockMovement> = {
    entityType: E.manufacturing.stock_movement,
  }

  // `manufacturing.stock_movement.created` (events.ts) — every movement, including
  ...
  async receive(lines: StockLine[], ref: StockMovementRef): Promise<{ movementIds: string[] }> {
    ...
    await withAtomicFlush(
      ...
      { transaction: true, label: 'manufacturing.stock.receive' },
    )
  ...
  async issue(lines: StockLine[], ref: StockMovementRef): Promise<{ movementIds: string[] }> {
    ...
    await withAtomicFlush(
      ...
      { transaction: true, label: 'manufacturing.stock.issue' },
    )
  ...
  async adjust(line: StockLine, reasonEntryId: string | null, ref: StockMovementRef): Promise<{ movementId: string }> {
    ...
    await withAtomicFlush(
      ...
      { transaction: true, label: 'manufacturing.stock.adjust' },
    )
  ...
  async reserve(lines: StockLine[], ref: StockMovementRef): Promise<{ reservationIds: string[] }> {
    ...
    await withAtomicFlush(
      ...
      { transaction: true, label: 'manufacturing.stock.reserve' },
    )
  ...
  async releaseReservations(ref: StockMovementRef): Promise<{ releasedIds: string[] }> {
    ...
    await withAtomicFlush(
      ...
      { transaction: true, label: 'manufacturing.stock.releaseReservations' },
    )
   ...
   * on-hand/batch state. Not part of the {@link ManufacturingStockProvider}
  ...
  async reverseMovement(movementId: string, scope: StockScope): Promise<{ movementId: string }> {
    ...
    try {
      await withAtomicFlush(
        ...
        { transaction: true, label: 'manufacturing.stock.reverseMovement' },
      )
    } catch (err) {
      ...
      // `reverses_movement_id` (`manufacturing_stock_movements_reverses_unique`)
