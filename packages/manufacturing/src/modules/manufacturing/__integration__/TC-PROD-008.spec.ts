 ...
 *   `admin` (which holds `manufacturing.operator.*` via the `manufacturing.*`
 ...
 *   `manufacturing.orders.view`/`manufacturing.reports.view` but NOT
 *   `manufacturing.operator.view`) gets 403 from the operator queue route,
 ...
 *   /api/manufacturing/orders" — the fixtures available make the *employee
...
test.describe('TC-PROD-008: Operator lite panel work queue + reporting', () => {
  ...
  test('operator panel: work-center tile -> queue -> partial report -> final report; operator-gated route rejects a non-operator token', async ({ page, request }) => {
    ...
    try {
      ...
      orderId = await createManufacturingOrder(request, adminToken, { productId, qtyPlanned: 5, uom: 'PCS' })
      await apiRequest(request, 'POST', `/api/manufacturing/orders/${orderId}/plan`, { token: adminToken })
      ...
      // --- Surface isolation: employee (no manufacturing.operator.view) is rejected ---
      const employeeQueueResponse = await apiRequest(
        request,
        'GET',
        `/api/manufacturing/operator/queue?workCenterId=${workCenterId}`,
        { token: employeeToken },
      )
      expect(employeeQueueResponse.status(), 'employee token lacks manufacturing.operator.view').toBe(403)
      ...
      // --- Partial report ---
      const partialResponse = await apiRequest(request, 'POST', '/api/manufacturing/reports', {
        token: adminToken,
      ...
      // --- Final report ---
      const finalResponse = await apiRequest(request, 'POST', '/api/manufacturing/reports', {
        token: adminToken,
      ...
      await page.goto('/backend/manufacturing/operator')
