 ...
 *
 * Self-contained: creates its own `manufacturing_enabled` toggle state (idempotent,
 * see helpers/manufacturing.ts) and its own work-center fixture, cleaning the
...
test.describe('TC-PROD-001: Work Center CRUD + list UI visibility', () => {
  test('creates, lists, updates, and deletes a work center via the API and sees it in the backend list', async ({ request, page }) => {
    ...
    try {
      ...
      const listResponse = await apiRequest(request, 'GET', `/api/manufacturing/work-centers?id=${workCenterId}`, { token: adminToken })
      ...
      const updateResponse = await apiRequest(request, 'PUT', '/api/manufacturing/work-centers', {
        token: adminToken,
        data: { id: workCenterId, costRatePerHour: 20 },
      })
      expect(updateResponse.status()).toBe(200)

      const afterUpdateResponse = await apiRequest(request, 'GET', `/api/manufacturing/work-centers?id=${workCenterId}`, { token: adminToken })
      ...
      await page.goto('/backend/manufacturing/work-centers')
      const row = page.getByRole('row').filter({ has: page.getByRole('cell', { name, exact: true }) })
      await expect(row).toBeVisible({ timeout: 10_000 })

      const deleteResponse = await apiRequest(request, 'DELETE', '/api/manufacturing/work-centers', {
        token: adminToken,
      ...
      const afterDeleteResponse = await apiRequest(request, 'GET', `/api/manufacturing/work-centers?id=${deletedId}`, { token: adminToken })
