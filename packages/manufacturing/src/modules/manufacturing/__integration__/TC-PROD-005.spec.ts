 ...
 *
 * Self-contained: uses `uniqueUuid()` fixture product ids (manufacturing APIs
...
test.describe('TC-PROD-005: Stock intake (CSV import, receipt, storno)', () => {
  test('CSV import (2 valid rows + 1 broken row) reports per-row results and on-hand becomes visible; storno restores prior balance', async ({ request }) => {
    ...
    const importResponse = await request.post('/api/manufacturing/stock/import', {
    ...
    const stockAfterImportResponse = await apiRequest(
      request,
      'GET',
      `/api/manufacturing/stock?productId=${productId}`,
      { token: adminToken },
    )
    ...
    const batchesResponse = await apiRequest(
      request,
      'GET',
      `/api/manufacturing/stock/batches?productId=${productId}`,
      { token: adminToken },
    )
    ...
    const reverseResponse = await apiRequest(
      request,
      'POST',
      `/api/manufacturing/stock/movements/${receiveMovementId}/reverse`,
      { token: adminToken },
    )
    ...
    const movementsResponse = await apiRequest(
      request,
      'GET',
      `/api/manufacturing/stock/movements?productId=${productId}&pageSize=20`,
      { token: adminToken },
    )
