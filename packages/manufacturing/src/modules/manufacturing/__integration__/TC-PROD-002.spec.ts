 ...
 * `finally`; the toggle is ensured idempotently (see helpers/manufacturing.ts).
 */
test.describe('TC-PROD-002: BOM create -> copy version -> activate -> cycle rejection', () => {
  test('copies a version and activates it, archiving the prior active version', async ({ request }) => {
    ...
    try {
      ...
      const activateResponse = await apiRequest(request, 'POST', `/api/manufacturing/boms/${bomId}/activate`, { token: adminToken })
      expect(activateResponse.status()).toBe(200)

      const copyResponse = await apiRequest(request, 'POST', `/api/manufacturing/boms/${bomId}/copy-version`, { token: adminToken })
      ...
      const listResponse = await apiRequest(request, 'GET', `/api/manufacturing/boms?productId=${productId}`, { token: adminToken })
      ...
      const activateCopyResponse = await apiRequest(request, 'POST', `/api/manufacturing/boms/${copyId}/activate`, { token: adminToken })
      expect(activateCopyResponse.status()).toBe(200)

      const afterActivateResponse = await apiRequest(request, 'GET', `/api/manufacturing/boms?productId=${productId}`, { token: adminToken })
  ...
  test('rejects activation that would create a circular bill of materials (422)', async ({ request }) => {
    ...
    try {
      ...
      const activateChildResponse = await apiRequest(request, 'POST', `/api/manufacturing/boms/${childBomId}/activate`, { token: adminToken })
