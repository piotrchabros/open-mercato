...
const MRP_RUN_QUEUE = 'manufacturing-mrp'
 ...
 * 069's own reasoning): the manufacturing Playwright harness does not run a
 * separate queue-worker process, so a real `MrpRun` created via
 * `POST /api/manufacturing/mrp/runs` would stay `pending` forever without
 * manual intervention. Rather than adding a new test-only trigger endpoint
 * to manufacturing code (no such admin/test hook exists here, unlike the
 ...
 * `TC-CRM-069`) to process the enqueued `manufacturing-mrp` job in-process. This
 ...
 * persisted `MrpSuggestion` rows for a run without any new manufacturing
 * surface area.
 *
...
async function waitForRunCompletion(
  ...
  while (Date.now() < deadline) {
    const response = await apiRequest(request, 'GET', `/api/manufacturing/mrp/runs?id=${runId}`, { token })
    if (response.ok()) {
...
test.describe('TC-PROD-009: MRP run + bulk accept/dismiss suggestions', () => {
  test('creates a run, drains the worker, lists/filters suggestions, and bulk accepts/dismisses them', async ({ request }) => {
    ...
    try {
      ...
      // --- Create the run ---
      const createResponse = await apiRequest(request, 'POST', '/api/manufacturing/mrp/runs', {
        token: adminToken,
        data: {},
      })
      expect(createResponse.status(), 'POST /api/manufacturing/mrp/runs should return 201').toBe(201)
      ...
      const listResponse = await apiRequest(
        request,
        'GET',
        `/api/manufacturing/mrp/runs/${runId}/suggestions?status=open&suggestionType=buy&pageSize=100`,
        { token: adminToken },
      )
      ...
      const acceptResponse = await apiRequest(request, 'POST', '/api/manufacturing/mrp/suggestions/accept', {
        token: adminToken,
        data: { ids: acceptIds },
      })
      ...
      // `buy` suggestions never create a draft manufacturing order (spec decision d).
      ...
      const reAcceptResponse = await apiRequest(request, 'POST', '/api/manufacturing/mrp/suggestions/accept', {
        token: adminToken,
        data: { ids: acceptIds },
      })
      ...
      // --- Bulk dismiss ---
      const dismissResponse = await apiRequest(request, 'POST', '/api/manufacturing/mrp/suggestions/dismiss', {
        token: adminToken,
        data: { ids: dismissIds },
      })
      ...
      const openAfterResponse = await apiRequest(
        request,
        'GET',
        `/api/manufacturing/mrp/runs/${runId}/suggestions?status=open&pageSize=100`,
        { token: adminToken },
      )
      ...
      const acceptedResponse = await apiRequest(
        request,
        'GET',
        `/api/manufacturing/mrp/runs/${runId}/suggestions?status=accepted&pageSize=100`,
        { token: adminToken },
      )
      ...
      const dismissedResponse = await apiRequest(
        request,
        'GET',
        `/api/manufacturing/mrp/runs/${runId}/suggestions?status=dismissed&pageSize=100`,
        { token: adminToken },
      )
