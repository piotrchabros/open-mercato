...
/**
 * Feature-toggle identifier gating the manufacturing module surface (mirrors
 * `packages/manufacturing/src/modules/manufacturing/lib/manufacturingToggleId.ts`).
 * Duplicated here (not imported) so this helper file has no compile-time
 * dependency on the manufacturing package's server-only module graph.
 */
export const PRODUCTION_TOGGLE_ID = 'manufacturing_enabled'
...
export function uniqueUuid(): string {
  ...
  // ids — the manufacturing APIs only require a well-formed UUID, they never
...
/**
 * Ensures the `manufacturing_enabled` global feature toggle exists and defaults
 * to `true`, so `isManufacturingEnabledForTenant` (fail-closed gate) resolves
 ...
 */
export async function ensureManufacturingEnabledToggle(
  ...
  const createResponse = await apiRequest(request, 'POST', '/api/feature_toggles/global', {
    ...
    data: {
      ...
      name: 'Manufacturing module enabled',
      description: 'Gates the manufacturing planning module backend UI (integration test fixture).',
  ...
  expect(found?.id, 'manufacturing_enabled toggle should exist after create-or-race').toBeTruthy()
...
export async function createWorkCenter(
  ...
  const response = await apiRequest(request, 'POST', '/api/manufacturing/work-centers', {
    token,
    data: {
  ...
  expect(response.status(), 'POST /api/manufacturing/work-centers should return 201').toBe(201)
...
export async function deleteWorkCenterIfExists(
  ...
  if (!token || !id) return
  await apiRequest(request, 'DELETE', '/api/manufacturing/work-centers', { token, data: { id } }).catch(() => null)
}
...
export async function createBom(
  ...
  const response = await apiRequest(request, 'POST', '/api/manufacturing/boms', {
    token,
    data: {
      productId,
  ...
  expect(response.status(), 'POST /api/manufacturing/boms should return 201').toBe(201)
  const body = await readJsonSafe<{ id?: string }>(response)
...
export async function deleteBomIfExists(
  ...
  if (!token || !id) return
  await apiRequest(request, 'DELETE', '/api/manufacturing/boms', { token, data: { id } }).catch(() => null)
}
...
export async function createRouting(
  ...
  const productId = overrides.productId ?? uniqueUuid()
  const response = await apiRequest(request, 'POST', '/api/manufacturing/routings', {
    token,
    data: {
      productId,
  ...
  expect(response.status(), 'POST /api/manufacturing/routings should return 201').toBe(201)
  const body = await readJsonSafe<{ id?: string }>(response)
...
export async function deleteRoutingIfExists(
  ...
  if (!token || !id) return
  await apiRequest(request, 'DELETE', '/api/manufacturing/routings', { token, data: { id } }).catch(() => null)
}
...
export async function createPlanningParams(
  ...
  const response = await apiRequest(request, 'POST', '/api/manufacturing/planning-params', {
    token,
    data: {
      productId: overrides.productId,
  ...
  expect(response.status(), 'POST /api/manufacturing/planning-params should return 201').toBe(201)
  const body = await readJsonSafe<{ id?: string }>(response)
...
export async function deletePlanningParamsIfExists(
  ...
  if (!token || !id) return
  await apiRequest(request, 'DELETE', '/api/manufacturing/planning-params', { token, data: { id } }).catch(() => null)
}

/**
 * Creates a manufacturing order (task 3.4). Mirrors the "Create manufacturing
 ...
 * happy path exercises — manufacturing APIs never resolve `sourceId` against
...
export async function createManufacturingOrder(
  ...
  const response = await apiRequest(request, 'POST', '/api/manufacturing/orders', {
    token,
    data: {
      productId: overrides.productId,
      variantId: overrides.variantId ?? null,
  ...
  expect(response.status(), 'POST /api/manufacturing/orders should return 201').toBe(201)
  const body = await readJsonSafe<{ id?: string }>(response)
  expect(typeof body?.id === 'string', 'manufacturing order creation response should include an id').toBe(true)
  return String(body!.id)
}

export async function deleteManufacturingOrderIfExists(
  request: APIRequestContext,
  token: string | null,
  id: string | null,
): Promise<void> {
  if (!token || !id) return
  await apiRequest(request, 'DELETE', '/api/manufacturing/orders', { token, data: { id } }).catch(() => null)
}
