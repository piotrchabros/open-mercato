 ...
 * (`commands/reports.ts`) resolves `ManufacturingOrderMaterial` snapshot rows +
 ...
 * `ManufacturingOrderMaterial.qtyRequired`. Task 3.1's release snapshot copies
 * `ManufacturingBomItem.qtyPerUnit` verbatim into `qtyRequired` (see
...
export type BackflushMaterialLine = {
  ...
  /** `ManufacturingOrderMaterial.qtyRequired` — see module doc for semantics. */
  qtyPerUnit: number
  scrapFactor: number
  uom: string
}
 ...
 * against. `ManufacturingBomItem.operationSequence` (copied verbatim into
 * `ManufacturingOrderMaterial.operationSequence` at release) assigns a material
 ...
 * `ManufacturingStockProvider.issue` (which does NOT convert itself, per
