 ...
 * Scope). Pure/unit-testable module fed by scoped `ManufacturingOrderMaterial`
 * (+ `ManufacturingOrderOperation`) rows — no ORM/em access here (same
 * convention as `lib/reports/lateOrders.ts`).
 *
 * Semantics of "standard" (review finding, task 6.1 R1 — corrects an
 * earlier version of this module): `ManufacturingOrderMaterial.qtyRequired` is
...
export type ConsumptionMaterialRow = {
  ...
  /** `ManufacturingOrderMaterial.qtyRequired` — material needed per ONE finished unit (see module doc). */
  qtyPerUnit: string
  scrapFactor: string
  qtyIssued: string
