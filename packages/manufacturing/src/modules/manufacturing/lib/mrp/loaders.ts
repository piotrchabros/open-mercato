...
import { ManufacturingBom, ManufacturingBomItem, ProductPlanningParams, StockItem, ManufacturingOrder } from '../../data/entities.js'
 ...
 * row (`StockItem`/`ProductPlanningParams`/`ManufacturingBom`/`ManufacturingOrder`
...
export async function loadMrpInputs(em: EntityManager, params: LoadMrpInputsParams): Promise<MrpInputs> {
  ...
  const [planningParams, stockItems, activeBoms, openOrders] = await Promise.all([
    ...
    em.find(ManufacturingBom, { ...scope, status: 'active' }),
    ...
    em.find(ManufacturingOrder, { ...scope, status: { $in: ['released', 'in_progress'] } }),
  ])

  const bomItems = activeBoms.length
    ? await em.find(ManufacturingBomItem, { ...scope, bomId: { $in: activeBoms.map((b) => b.id) } })
    : []
