...
/**
 * `manufacturingStockProvider` DI seam (spec § Decisions, row i).
 *
 * Only this interface and the events it emits (`manufacturing.stock_movement.created`)
 * are contract surfaces — the `manufacturing_stock_*` tables backing the default
 * implementation are an internal detail. A future warehouse module can register
 * its own implementation under the same DI token (`manufacturingStockProvider`)
 * with a one-time data migration from these tables; the manufacturing module
 ...
 * operator MUST translate a `manufacturing.errors.*` key using the structured
 ...
 * `findBatches`.
 */
export interface ManufacturingStockProvider {
   ...
   * A caller that reserves material and later issues it (e.g. a manufacturing
