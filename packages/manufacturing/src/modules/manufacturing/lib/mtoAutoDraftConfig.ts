 ...
 * explicitly turn this on before the subscriber drafts any manufacturing order
...
export async function isMtoAutoDraftEnabled(resolver: Resolver, tenantId: string | null | undefined): Promise<boolean> {
  ...
  try {
    ...
    const value = await service.getValue<boolean>('manufacturing', MTO_AUTO_DRAFT_CONFIG_KEY, {
      defaultValue: false,
      scope: { tenantId },
    })
    return value === true
