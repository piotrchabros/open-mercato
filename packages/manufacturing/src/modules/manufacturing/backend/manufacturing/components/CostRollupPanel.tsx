 ...
 * Contracts, task 1.4). Calls `GET /api/manufacturing/boms/[id]/cost-rollup`
...
export function CostRollupPanel({ bomId, t }: { bomId: string; t: ReturnType<typeof useT> }) {
  ...
  const runRollup = React.useCallback(async () => {
    ...
    setError(null)
    try {
      const call = await apiCall<CostRollupResponse>(`/api/manufacturing/boms/${bomId}/cost-rollup`)
      if (call.ok && call.result) {
        setResult(call.result)
      } else {
        setError(t('manufacturing.boms.cost_rollup.error', 'Failed to compute cost rollup'))
      }
    } catch {
      setError(t('manufacturing.boms.cost_rollup.error', 'Failed to compute cost rollup'))
    } finally {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>{t('manufacturing.boms.cost_rollup.title', 'Cost estimate (catalog list prices)')}</CardTitle>
      </CardHeader>
      <CardContent className="flex flex-col gap-4">
        <div className="text-xs text-muted-foreground">
          {t(
            'manufacturing.boms.cost_rollup.caption',
        ...
        <Button type="button" variant="outline" size="sm" onClick={runRollup} disabled={loading}>
          {loading
            ? t('manufacturing.boms.cost_rollup.loading', 'Calculating...')
            : t('manufacturing.boms.cost_rollup.action', 'Calculate cost estimate')}
        </Button>
        ...
        {result && (
          <div className="flex flex-col gap-3">
            <div className="grid grid-cols-2 gap-2 text-sm sm:grid-cols-4">
              <div>
                <div className="text-muted-foreground">{t('manufacturing.boms.cost_rollup.materials', 'Materials')}</div>
                <div className="font-semibold">{result.materials.toFixed(2)} {result.currency ?? ''}</div>
              </div>
              <div>
                <div className="text-muted-foreground">{t('manufacturing.boms.cost_rollup.labor', 'Labor')}</div>
                <div className="font-semibold">{result.labor.toFixed(2)} {result.currency ?? ''}</div>
              </div>
              <div>
                <div className="text-muted-foreground">{t('manufacturing.boms.cost_rollup.total', 'Total')}</div>
                <div className="font-semibold">{result.total.toFixed(2)} {result.currency ?? ''}</div>
              </div>
              <div>
                <div className="text-muted-foreground">{t('manufacturing.boms.cost_rollup.per_unit', 'Per unit')}</div>
            ...
            {result.missingRouting && (
              <Alert status="warning">
                {t('manufacturing.boms.cost_rollup.missing_routing', 'No matching routing version was found; labor cost is 0.')}
              </Alert>
            )}
            {result.missingPrices.length > 0 && (
              <Alert status="warning">
                {t('manufacturing.boms.cost_rollup.missing_prices', 'Missing catalog prices for: {items}', {
            ...
            {result.missingConversions.length > 0 && (
              <Alert status="warning">
                {t('manufacturing.boms.cost_rollup.missing_conversions', 'Missing UoM conversions for: {items}', {
            ...
            {result.mixedCurrency.length > 0 && (
              <Alert status="warning">
                {t('manufacturing.boms.cost_rollup.mixed_currency', 'Mixed-currency prices excluded from the total for: {items}', {
