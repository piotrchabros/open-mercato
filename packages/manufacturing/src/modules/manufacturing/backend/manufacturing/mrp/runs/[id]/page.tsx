...
export default function MrpRunSuggestionsPage({ params }: { params?: { id?: string } }) {
  ...
  const bulkMutationContextId = 'manufacturing-mrp-suggestions:bulk-action'
  ...
  const singleMutationContextId = 'manufacturing-mrp-suggestions:single-action'
  ...
  React.useEffect(() => {
    ...
    async function loadRun() {
      ...
      setRunLoading(true)
      try {
        const call = await apiCall<{ items: MrpRunRow[] }>(`/api/manufacturing/mrp/runs?id=${runId}`)
        if (cancelled) return
        if (call.ok && call.result && call.result.items.length > 0) {
          setRun(call.result.items[0])
        } else if (!call.ok) {
          setRunError(t('manufacturing.mrp.error.run_load_failed', 'Failed to load MRP run'))
        } else {
          setIsNotFound(true)
        }
      } catch {
        if (!cancelled) setRunError(t('manufacturing.mrp.error.run_load_failed', 'Failed to load MRP run'))
      } finally {
    ...
    async function loadSuggestions() {
      ...
      try {
        ...
        const call = await apiCall<SuggestionsResponsePayload>(
          `/api/manufacturing/mrp/runs/${runId}/suggestions?${params.toString()}`,
          undefined,
          { fallback },
        )
        if (!call.ok) {
          if (!cancelled) flash(t('manufacturing.mrp.error.suggestions_load_failed', 'Failed to load MRP suggestions'), 'error')
          return
        }
      ...
      const result = await runBulkMutation({
        operation: async () => {
          const call = await apiCall<{ acceptedIds: string[]; createdOrderIds: string[]; skippedIds: string[] }>(
            '/api/manufacturing/mrp/suggestions/accept',
            { method: 'POST', body: JSON.stringify({ ids }) },
          )
          if (!call.ok) {
            throw Object.assign(new Error('[internal] manufacturing.mrp.acceptSuggestions failed'), {
              status: call.status,
        ...
        context: { formId: bulkMutationContextId, resourceKind: 'manufacturing.mrp_suggestion', retryLastMutation: retryBulkMutation },
      })
      return result
    },
      ...
      const result = await runBulkMutation({
        operation: async () => {
          const call = await apiCall<{ dismissedIds: string[]; skippedIds: string[] }>(
            '/api/manufacturing/mrp/suggestions/dismiss',
            { method: 'POST', body: JSON.stringify({ ids }) },
          )
          if (!call.ok) {
            throw Object.assign(new Error('[internal] manufacturing.mrp.dismissSuggestions failed'), {
              status: call.status,
              ...((call.result as Record<string, unknown> | null) ?? {}),
            })
          }
  ...
  const handleBulkAccept = React.useCallback(
    async (selectedRows: MrpSuggestionRow[]) => {
      const confirmed = await confirm({
        title: t('manufacturing.mrp.confirm.bulk_accept', 'Accept {count} suggestions?', { count: selectedRows.length }),
      })
      ...
      try {
        ...
        if (skippedCount > 0) {
          flash(
            t('manufacturing.mrp.success.bulk_accept_partial', '{accepted} accepted, {skipped} already resolved (skipped)', {
          ...
          flash(t('manufacturing.mrp.success.bulk_accept', '{count} suggestions accepted', { count: acceptedCount }), 'success')
        }
        setReloadToken((prev) => prev + 1)
        return true
      } catch (err) {
        const serverMessage = extractServerErrorMessage(err)
        flash(serverMessage ?? t('manufacturing.mrp.error.accept_failed', 'Failed to accept suggestions'), 'error')
        return false
      }
    },
    [acceptIds, confirm, t],
  ...
  const handleBulkDismiss = React.useCallback(
    async (selectedRows: MrpSuggestionRow[]) => {
      const confirmed = await confirm({
        title: t('manufacturing.mrp.confirm.bulk_dismiss', 'Dismiss {count} suggestions?', { count: selectedRows.length }),
      })
      if (!confirmed) return false
      try {
        ...
        if (skippedCount > 0) {
          flash(
            t('manufacturing.mrp.success.bulk_dismiss_partial', '{dismissed} dismissed, {skipped} already resolved (skipped)', {
          ...
          flash(t('manufacturing.mrp.success.bulk_dismiss', '{count} suggestions dismissed', { count: dismissedCount }), 'success')
        }
        setReloadToken((prev) => prev + 1)
        return true
      } catch (err) {
        const serverMessage = extractServerErrorMessage(err)
        flash(serverMessage ?? t('manufacturing.mrp.error.dismiss_failed', 'Failed to dismiss suggestions'), 'error')
        return false
      }
    },
      ...
      try {
        await runSingleMutation({
          ...
          context: {
            ...
            resourceKind: 'manufacturing.mrp_suggestion',
            resourceId: row.id,
            retryLastMutation: retrySingleMutation,
          },
        })
        flash(t('manufacturing.mrp.success.accept', 'Suggestion accepted'), 'success')
        setReloadToken((prev) => prev + 1)
      } catch (err) {
        const serverMessage = extractServerErrorMessage(err)
        flash(serverMessage ?? t('manufacturing.mrp.error.accept_failed', 'Failed to accept suggestions'), 'error')
  ...
  const handleSingleDismiss = React.useCallback(
    async (row: MrpSuggestionRow) => {
      try {
        ...
        flash(t('manufacturing.mrp.success.dismiss', 'Suggestion dismissed'), 'success')
        setReloadToken((prev) => prev + 1)
      } catch (err) {
        const serverMessage = extractServerErrorMessage(err)
        flash(serverMessage ?? t('manufacturing.mrp.error.dismiss_failed', 'Failed to dismiss suggestions'), 'error')
  ...
  const handleExportBuyCsv = React.useCallback(() => {
    ...
    window.open('/api/manufacturing/mrp/suggestions/export', '_blank', 'noopener,noreferrer')
  }, [])
    ...
    () => [
      {
        ...
        header: t('manufacturing.mrp.suggestion.field.type', 'Type'),
        enableSorting: false,
        cell: ({ row }) => <MrpSuggestionTypeBadge type={row.original.suggestionType} t={t} />,
      },
      { id: 'productId', accessorKey: 'productId', header: t('manufacturing.mrp.suggestion.field.product_id', 'Product') },
      {
        id: 'qty',
        accessorKey: 'qty',
        header: t('manufacturing.mrp.suggestion.field.qty', 'Qty (UoM)'),
        enableSorting: false,
      ...
      },
      {
        id: 'dueDate',
        accessorKey: 'dueDate',
        header: t('manufacturing.mrp.suggestion.field.due_date', 'Due date'),
        cell: ({ row }) => new Date(row.original.dueDate).toLocaleDateString(),
      },
      {
        id: 'status',
        accessorKey: 'status',
        header: t('manufacturing.mrp.suggestion.field.status', 'Status'),
        enableSorting: false,
        cell: ({ row }) => <MrpSuggestionStatusBadge status={row.original.status} t={t} />,
      },
      {
        id: 'pegging',
        header: t('manufacturing.mrp.suggestion.field.pegging', 'Demand source'),
        enableSorting: false,
        cell: ({ row }) => {
          ...
          return (
            <SimpleTooltip content={<span className="whitespace-pre-line">{summary}</span>}>
              <span className="cursor-help underline decoration-dotted">
                {t('manufacturing.mrp.suggestion.pegging_count', '{count} demand source(s)', {
  ...
  const filterDefs = React.useMemo<FilterDef[]>(
    () => [
      {
        id: 'status',
        label: t('manufacturing.mrp.suggestion.filters.status', 'Status'),
        type: 'select',
        options: SUGGESTION_STATUSES.map((status) => ({
          value: status,
          label: t(`manufacturing.mrp.suggestion.status.${status}`, status),
        })),
      },
      {
        id: 'suggestionType',
        label: t('manufacturing.mrp.suggestion.filters.type', 'Type'),
        type: 'select',
        options: [
          { label: t('manufacturing.mrp.suggestion.filters.all', 'All'), value: '' },
          ...SUGGESTION_TYPES.map((suggestionType) => ({
            value: suggestionType,
            label: t(`manufacturing.mrp.suggestion.type.${suggestionType}`, suggestionType),
          })),
        ],
      },
    ],
  ...
  const bulkActions = React.useMemo<BulkAction<MrpSuggestionRow>[]>(
    () => [
      {
        id: 'accept',
        label: t('manufacturing.mrp.action.bulk_accept', 'Accept selected'),
        onExecute: handleBulkAccept,
      },
      {
        id: 'dismiss',
        label: t('manufacturing.mrp.action.bulk_dismiss', 'Dismiss selected'),
        destructive: true,
  ...
  if (runLoading) {
    return (
      <Page>
        <PageBody>
          <LoadingMessage label={t('manufacturing.mrp.loading', 'Loading MRP run...')} />
        </PageBody>
      </Page>
    )
  }
  ...
  if (isNotFound) {
    return (
      <Page>
        <PageBody>
          <RecordNotFoundState
            label={t('manufacturing.mrp.error.run_not_found', 'MRP run not found')}
            backHref="/backend/manufacturing/mrp"
            backLabel={t('manufacturing.mrp.title', 'MRP Runs')}
          />
        </PageBody>
      </Page>
    )
  ...
  if (runError || !run) {
    return (
      <Page>
        <PageBody>
          <ErrorMessage label={runError ?? t('manufacturing.mrp.error.run_not_found', 'MRP run not found')} />
        </PageBody>
      </Page>
    )
  }
      ...
      <PageBody>
        <div className="mb-4 flex items-center gap-3">
          ...
          <span className="text-sm text-muted-foreground">
            {t('manufacturing.mrp.run.detail_subtitle', 'Started {started} / Finished {finished}', {
        ...
        <DataTable<MrpSuggestionRow>
          title={t('manufacturing.mrp.suggestions.title', 'MRP Run Suggestions')}
          columns={columns}
          data={rows}
          filters={filterDefs}
          ...
          actions={
            <Button variant="outline" onClick={handleExportBuyCsv}>
              ...
              {t('manufacturing.mrp.action.export_buy_csv', 'Export buy CSV')}
            </Button>
          }
          rowActions={(row) => (
            <RowActions
              items={[
                {
                  id: 'accept',
                  label: t('manufacturing.mrp.action.accept', 'Accept'),
                  onSelect: () => handleSingleAccept(row),
                },
                {
                  id: 'dismiss',
                  label: t('manufacturing.mrp.action.dismiss', 'Dismiss'),
                  destructive: true,
          ...
          emptyState={(
            <ListEmptyState
              entityName={t('manufacturing.mrp.suggestions.title', 'MRP Run Suggestions')}
              description={t('manufacturing.mrp.suggestions.empty_description', 'No open suggestions for this run.')}
            />
          )}
          refreshButton={{
            label: t('manufacturing.mrp.action.refresh', 'Refresh'),
          ...
          isLoading={isLoading}
        />
        <Button variant="ghost" className="mt-4" onClick={() => router.push('/backend/manufacturing/mrp')}>
          {t('manufacturing.mrp.action.back_to_runs', 'Back to MRP Runs')}
        </Button>
