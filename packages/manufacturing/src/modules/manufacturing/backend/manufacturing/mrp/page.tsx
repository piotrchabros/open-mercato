 ...
 * (`manufacturing.mrp.createRun` — one `MrpRun` + one per-tenant queue job,
...
export default function MrpRunsPage() {
  ...
  const mutationContextId = 'manufacturing-mrp-runs:create-run'
  ...
  React.useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const call = await apiCall<ResponsePayload>(`/api/manufacturing/mrp/runs?${params.toString()}`, undefined, { fallback })
        if (!call.ok) {
          if (!cancelled) flash(t('manufacturing.mrp.error.list_failed', 'Failed to load MRP runs'), 'error')
          return
        }
  ...
  const handleRunNow = React.useCallback(async () => {
    ...
    try {
      await runMutation({
        operation: async () => {
          const result = await apiCall('/api/manufacturing/mrp/runs', { method: 'POST', body: JSON.stringify({}) })
          if (!result.ok) {
            throw Object.assign(new Error('[internal] manufacturing.mrp.createRun failed'), {
        ...
        context: { formId: mutationContextId, resourceKind: 'manufacturing.mrp_run', retryLastMutation },
      })
      flash(t('manufacturing.mrp.success.run_started', 'MRP run started. Progress appears in the top bar.'), 'success')
      setReloadToken((prev) => prev + 1)
    } catch (err) {
      const serverMessage = extractServerErrorMessage(err)
      flash(serverMessage ?? t('manufacturing.mrp.error.run_start_failed', 'Failed to start MRP run'), 'error')
    } finally {
    ...
    () => [
      {
        id: 'status',
        accessorKey: 'status',
        header: t('manufacturing.mrp.run.field.status', 'Status'),
        enableSorting: false,
      ...
      },
      {
        ...
        header: t('manufacturing.mrp.run.field.started_at', 'Started'),
        cell: ({ row }) => (row.original.startedAt ? new Date(row.original.startedAt).toLocaleString() : '—'),
      },
      {
        id: 'finishedAt',
        accessorKey: 'finishedAt',
        header: t('manufacturing.mrp.run.field.finished_at', 'Finished'),
        cell: ({ row }) => (row.original.finishedAt ? new Date(row.original.finishedAt).toLocaleString() : '—'),
      },
      {
        id: 'stats',
        accessorKey: 'stats',
        header: t('manufacturing.mrp.run.field.stats', 'Stats'),
        enableSorting: false,
        cell: ({ row }) => {
          const stats = row.original.stats
          if (!stats) return '—'
          return t(
            'manufacturing.mrp.run.stats_summary',
      ...
      },
      {
        ...
        header: t('manufacturing.mrp.run.field.created_at', 'Created'),
  ...
  const filterDefs = React.useMemo<FilterDef[]>(
    () => [
      {
        id: 'status',
        label: t('manufacturing.mrp.run.filters.status', 'Status'),
        type: 'select',
        multiple: true,
        options: RUN_STATUSES.map((status) => ({
          value: status,
          label: t(`manufacturing.mrp.run.status.${status}`, status),
        })),
      },
    ],
    [t],
  ...
  return (
    <Page>
      <PageBody>
        <DataTable<MrpRunRow>
          title={t('manufacturing.mrp.title', 'MRP Runs')}
          columns={columns}
          data={rows}
          filters={filterDefs}
          ...
          actions={
            <Button onClick={handleRunNow} disabled={isStartingRun}>
              ...
              {isStartingRun
                ? t('manufacturing.mrp.action.starting', 'Starting…')
                : t('manufacturing.mrp.action.run_now', 'Run MRP now')}
            </Button>
          }
          onRowClick={(row) => router.push(`/backend/manufacturing/mrp/runs/${row.id}`)}
          rowActions={(row) => (
            <RowActions
              items={[
                {
                  id: 'view',
                  label: t('manufacturing.mrp.action.view', 'View suggestions'),
                  href: `/backend/manufacturing/mrp/runs/${row.id}`,
                },
              ]}
            />
          )}
          emptyState={(
            <ListEmptyState
              entityName={t('manufacturing.mrp.title', 'MRP Runs')}
              createLabel={t('manufacturing.mrp.action.run_now', 'Run MRP now')}
