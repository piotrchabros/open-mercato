...
export default function EditWorkCenterPage({ params }: { params?: { id?: string } }) {
  ...
  React.useEffect(() => {
    ...
    async function load() {
      try {
        const call = await apiCall<{ items: WorkCenterData[] }>(`/api/manufacturing/work-centers?id=${params?.id}`)
        if (cancelled) return
        if (call.ok && call.result && call.result.items.length > 0) {
          setRecord(call.result.items[0])
        } else if (!call.ok) {
          setError(t('manufacturing.work_centers.error.load_failed', 'Failed to load work center'))
        } else {
          setIsNotFound(true)
        }
      } catch {
        if (!cancelled) setError(t('manufacturing.work_centers.error.load_failed', 'Failed to load work center'))
      } finally {
  ...
  const fields = React.useMemo<CrudField[]>(
    () => [
      { id: 'name', type: 'text', label: t('manufacturing.work_centers.field.name', 'Name'), required: true },
      {
        id: 'kind',
        type: 'select',
      ...
      { id: 'isActive', type: 'checkbox', label: t('manufacturing.work_centers.field.is_active', 'Active') },
    ],
    [t],
  )
  ...
  const handleDelete = React.useCallback(async () => {
    ...
    try {
      ...
      await deleteCrud('manufacturing/work-centers', { id: record.id })
  ...
  if (loading) {
    return (
      <Page>
        <PageBody>
          <LoadingMessage label={t('manufacturing.work_centers.loading', 'Loading work center...')} />
        </PageBody>
      </Page>
    )
  }
  ...
  if (isNotFound) {
    return (
      <Page>
        <PageBody>
          <RecordNotFoundState
            label={t('manufacturing.work_centers.error.not_found', 'Work center not found')}
            backHref="/backend/manufacturing/work-centers"
            backLabel={t('manufacturing.work_centers.title', 'Work Centers')}
          />
        </PageBody>
      </Page>
    )
  ...
  if (error || !record) {
    return (
      <Page>
        <PageBody>
          <ErrorMessage label={error ?? t('manufacturing.work_centers.error.not_found', 'Work center not found')} />
        </PageBody>
      </Page>
    )
  }
    ...
    <Page>
      <PageBody>
        <CrudForm<WorkCenterCreateInput>
          title={t('manufacturing.work_centers.edit.title', 'Edit work center')}
          backHref="/backend/manufacturing/work-centers"
          fields={fields}
          ...
          submitLabel={t('manufacturing.work_centers.form.save', 'Save changes')}
          cancelHref="/backend/manufacturing/work-centers"
          onDelete={handleDelete}
          onSubmit={async (values) => {
            try {
              await updateCrud('manufacturing/work-centers', { id: record.id, ...values })
            } catch (err) {
              if (surfaceRecordConflict(err, t)) return
              throw err
            }
            flash(t('manufacturing.work_centers.success.updated', 'Work center updated successfully'), 'success')
            router.push('/backend/manufacturing/work-centers')
          }}
        />
      </PageBody>
