...
export default function EditBomPage({ params }: { params?: { id?: string } }) {
  ...
  const loadRecord = React.useCallback(async () => {
    ...
    setLoading(true)
    try {
      const call = await apiCall<BomDetail>(`/api/manufacturing/boms/${params.id}`)
      if (call.ok && call.result) {
        setRecord(call.result)
      } else if (call.status === 404) {
        setIsNotFound(true)
      } else {
        setError(t('manufacturing.boms.error.load_failed', 'Failed to load BOM'))
      }
    } catch {
      setError(t('manufacturing.boms.error.load_failed', 'Failed to load BOM'))
    } finally {
  ...
  const handleDelete = React.useCallback(async () => {
    ...
    try {
      ...
      await deleteCrud('manufacturing/boms', { id: record.id })
  ...
  if (loading) {
    return (
      <Page>
        <PageBody>
          <LoadingMessage label={t('manufacturing.boms.loading', 'Loading BOM...')} />
        </PageBody>
      </Page>
    )
  }
  ...
  if (isNotFound) {
    return (
      <Page>
        <PageBody>
          <RecordNotFoundState
            label={t('manufacturing.boms.error.not_found', 'BOM not found')}
            backHref="/backend/manufacturing/boms"
            backLabel={t('manufacturing.boms.title', 'Bills of Materials')}
          />
        </PageBody>
      </Page>
    )
  ...
  if (error || !record) {
    return (
      <Page>
        <PageBody>
          <ErrorMessage label={error ?? t('manufacturing.boms.error.not_found', 'BOM not found')} />
        </PageBody>
      </Page>
    )
  }
    ...
    <Page>
      <PageBody>
        ...
        <CrudForm<BomEditValues>
          title={t('manufacturing.boms.edit.title', 'Edit BOM')}
          backHref="/backend/manufacturing/boms"
          fields={fields}
          ...
          submitLabel={t('manufacturing.boms.form.save', 'Save changes')}
          cancelHref="/backend/manufacturing/boms"
          onDelete={handleDelete}
          onSubmit={async (values) => {
            try {
              await updateCrud('manufacturing/boms', { id: record.id, ...values })
            } catch (err) {
              if (surfaceRecordConflict(err, t)) return
              throw err
            }
            flash(t('manufacturing.boms.success.updated', 'BOM updated successfully'), 'success')
            router.push('/backend/manufacturing/boms')
          }}
        />
      </PageBody>
