...
import { ProductPlanningParams, ManufacturingOrder } from '../data/entities.js'
import { isMtoAutoDraftEnabled } from '../lib/mtoAutoDraftConfig.js'

const logger = createLogger('manufacturing').child({ component: 'sales-order-created-mto' })

/**
 * Optional sales -> manufacturing soft integration (spec § Sales integration).
 ...
 * `manufacturing.mto_auto_draft` (OFF by default), drafts a manufacturing order
...
export const metadata = {
  ...
  persistent: true,
  id: 'manufacturing:sales-order-mto-draft',
}
...
export default async function handle(payload: SalesOrderCreatedPayload, ctx: ResolverContext): Promise<void> {
  try {
    ...
    const existingDrafts = await fork.find(ManufacturingOrder, {
      tenantId,
      organizationId,
    ...
    for (const line of makeLines) {
      ...
      try {
        await commandBus.execute('manufacturing.orders.create', {
          input: {
        ...
        logger.error('manufacturing.sales-order-mto-draft.line_failed', { err, orderId, productId: line.productId })
      }
    }
  } catch (err) {
    logger.error('manufacturing.sales-order-mto-draft.failed', { err })
  }
}
