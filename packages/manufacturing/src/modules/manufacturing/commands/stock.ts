 ...
 * exactly what `manufacturing.stock.reverseMovement` (storno) is. Per
 ...
 * `ManufacturingStockProvider` method (`receive`/`adjust`/`reverseMovement`) is
 * itself already responsible for emitting `manufacturing.stock_movement.created`
 ...
 * `manufacturingStockProvider` seam — resolved via the container, never
...
/**
 * `reverseMovement` is intentionally NOT part of the `ManufacturingStockProvider`
...
async function mapStockProviderError(err: unknown): Promise<unknown> {
  ...
  if (err instanceof StockUomMismatchError) {
    return new CrudHttpError(422, {
      error: translate(
        'manufacturing.errors.stock_uom_mismatch',
  ...
  if (err instanceof InsufficientStockError) {
    return new CrudHttpError(422, {
      error: translate('manufacturing.errors.insufficient_stock', 'Insufficient stock for this operation.'),
    })
  }
  if (err instanceof DoubleReversalError) {
    return new CrudHttpError(409, {
      error: translate('manufacturing.errors.double_reversal', 'This stock movement has already been reversed.'),
    })
  }
  return err
}
...
// ---------------------------------------------------------------------------
// manufacturing.stock.receive — manual receipt (PW: goods-received-note style)
// ---------------------------------------------------------------------------

const receiveStockCommand: CommandHandler<StockReceiveInput, { movementIds: string[] }> = {
  id: 'manufacturing.stock.receive',
  isUndoable: false,
  ...
  async buildLog({ input, result, ctx }) {
    ...
    const { tenantId, organizationId } = requireScopeIds(ctx)
    return {
      actionLabel: translate('manufacturing.audit.stock.receive', 'Receive stock'),
      resourceKind: 'manufacturing.stock_movement',
...
// ---------------------------------------------------------------------------
// manufacturing.stock.adjust — opening balance load / correction (signed qty)
// ---------------------------------------------------------------------------

const adjustStockCommand: CommandHandler<StockAdjustInput, { movementId: string }> = {
  id: 'manufacturing.stock.adjust',
  isUndoable: false,
  ...
  async buildLog({ input, result, ctx }) {
    ...
    const { tenantId, organizationId } = requireScopeIds(ctx)
    return {
      actionLabel: translate('manufacturing.audit.stock.adjust', 'Adjust stock'),
      resourceKind: 'manufacturing.stock_movement',
...
// ---------------------------------------------------------------------------
// manufacturing.stock.reverseMovement — storno (compensating movement)
// ---------------------------------------------------------------------------

const reverseStockMovementCommand: CommandHandler<StockReverseMovementInput, { movementId: string }> = {
  id: 'manufacturing.stock.reverseMovement',
  isUndoable: false,
  ...
  async buildLog({ input, result, ctx }) {
    ...
    const { tenantId, organizationId } = requireScopeIds(ctx)
    return {
      actionLabel: translate('manufacturing.audit.stock.reverse_movement', 'Reverse stock movement'),
      resourceKind: 'manufacturing.stock_movement',
      resourceId: result.movementId,
      tenantId,
      organizationId,
      relatedResourceKind: 'manufacturing.stock_movement',
