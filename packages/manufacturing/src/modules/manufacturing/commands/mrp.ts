 ...
 *
 * `manufacturing.mrp.createRun` and `manufacturing.mrp.cronFanOut` never execute
 * the engine synchronously — they only create the `MrpRun` row(s) and
 * enqueue one queue job per row onto `manufacturing-mrp` (spec decision c). The
...
const mrpSuggestionCrudIndexer: CrudIndexerConfig<MrpSuggestion> = { entityType: E.manufacturing.mrp_suggestion }
...
// ---------------------------------------------------------------------------
// manufacturing.mrp.createRun — one MrpRun + one queue job (spec decision c)
// ---------------------------------------------------------------------------

const createRunCommand: CommandHandler<MrpRunCreateInput, { id: string }> = {
  id: 'manufacturing.mrp.createRun',
  isUndoable: false,
  ...
  async buildLog({ result }) {
    ...
    return {
      actionLabel: translate('manufacturing.audit.mrp.create_run', 'Start MRP run'),
      resourceKind: 'manufacturing.mrp_run',
...
// ---------------------------------------------------------------------------
// manufacturing.mrp.cronFanOut — scheduler target command (system scope)
//
...
// `targetCommand: 'manufacturing.mrp.cronFanOut'`.
//
...
const cronFanOutCommand: CommandHandler<Record<string, unknown>, { runsCreated: number }> = {
  id: 'manufacturing.mrp.cronFanOut',
  isUndoable: false,
  ...
  async execute(_input, ctx) {
    ...
    const rows = await connection.execute(
      `select distinct "tenant_id", "organization_id" from "manufacturing_planning_params" where "deleted_at" is null`,
      [],
      'all',
    )
    ...
    for (const scope of scopes) {
      ...
      // including cron. A tenant with `manufacturing_enabled=false` (default —
      // `isManufacturingEnabledForTenant` fails closed) is skipped entirely: no
      // `MrpRun` row, no queue job, even if it has `ProductPlanningParams`
      // rows from before the toggle was disabled.
      const enabled = await isManufacturingEnabledForTenant(scope.tenantId)
      if (!enabled) continue
  ...
  async buildLog({ result }) {
    const { translate } = await resolveTranslations()
    return {
      actionLabel: translate('manufacturing.audit.mrp.cron_fan_out', 'MRP cron fan-out'),
      resourceKind: 'manufacturing.mrp_run',
      resourceId: null,
...
// ---------------------------------------------------------------------------
// manufacturing.mrp.acceptSuggestions — bulk accept
//
// `make` -> creates a draft manufacturing order (`manufacturing.orders.create`,
// `sourceType: 'mrp'`, `sourceId: suggestion.id`).
// `buy` -> marks accepted and emits `manufacturing.mrp_suggestion.accepted`
...
const acceptSuggestionsCommand: CommandHandler<MrpSuggestionsBulkActionInput, AcceptSuggestionsResult> = {
  id: 'manufacturing.mrp.acceptSuggestions',
  isUndoable: false,
  ...
  async execute(input, ctx) {
    ...
    for (const row of rows) {
      ...
      await emitManufacturingEvent(
        'manufacturing.mrp_suggestion.accepted',
        {
          id: row.id,
  ...
  async buildLog({ result }) {
    const { translate } = await resolveTranslations()
    return {
      actionLabel: translate('manufacturing.audit.mrp.accept_suggestions', 'Accept MRP suggestions'),
...
// ---------------------------------------------------------------------------
// manufacturing.mrp.dismissSuggestions — bulk dismiss
// ---------------------------------------------------------------------------
...
const dismissSuggestionsCommand: CommandHandler<MrpSuggestionsBulkActionInput, DismissSuggestionsResult> = {
  id: 'manufacturing.mrp.dismissSuggestions',
  isUndoable: false,
  ...
  async buildLog({ result }) {
    const { translate } = await resolveTranslations()
    return {
      actionLabel: translate('manufacturing.audit.mrp.dismiss_suggestions', 'Dismiss MRP suggestions'),
      resourceKind: 'manufacturing.mrp_suggestion',
