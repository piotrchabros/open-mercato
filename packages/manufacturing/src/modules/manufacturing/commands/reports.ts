 ...
 * task 4.1).
 *
 * `ManufacturingReport` is append-only + storno (decision h) — see the entity
 ...
 * second gets a translated conflict"): reports are a `ManufacturingOrder`
 * sub-resource guarded by the PARENT order's `updated_at`
 * (`enforceManufacturingOrderOptimisticLock`, spec § Status machine — same
...
function resolveStockLedger(ctx: CommandRuntimeContext): Pick<StockLedgerService, 'reverseMovement'> {
  return ctx.container.resolve<Pick<StockLedgerService, 'reverseMovement'>>('manufacturingStockProvider')
}

const reportCrudIndexer: CrudIndexerConfig<ManufacturingReport> = { entityType: E.manufacturing.manufacturing_report }
const reportCrudEvents: CrudEventsConfig<ManufacturingReport> = { module: 'manufacturing', entity: 'report', persistent: true }
 ...
 * (no ORM relation is declared on any manufacturing entity), which is the
...
function toMaterialLines(materials: ManufacturingOrderMaterial[]): BackflushMaterialLine[] {
...
async function runBackflush(
  ...
  const allMaterials = await em.find(ManufacturingOrderMaterial, { orderId: order.id, deletedAt: null })
  ...
  const dirtyMaterials: ManufacturingOrderMaterial[] = []
  ...
  if (dirtyMaterials.length > 0) {
    await withAtomicFlush(em, [() => {}], { transaction: true, label: 'manufacturing.reports.backflush' })
  }
...
const createReportCommand: CommandHandler<ReportCreateInput, ReportCreateResult> = {
  id: 'manufacturing.reports.create',
  isUndoable: false,
  ...
  async execute(input, ctx) {
    ...
    const operation = await em.findOne(ManufacturingOrderOperation, {
    ...
    if (!operation) {
      throw new CrudHttpError(404, { error: '[internal] Manufacturing order operation not found' })
    }

    const order = await em.findOne(ManufacturingOrder, { id: operation.orderId, tenantId, organizationId, deletedAt: null })
    if (!order) {
      throw new CrudHttpError(404, { error: '[internal] Manufacturing order not found' })
    }
    ...
    if (order.status !== 'released' && order.status !== 'in_progress') {
      throw new CrudHttpError(422, {
        error: translate(
          'manufacturing.errors.report_order_not_active',
          'Reports can only be recorded while the manufacturing order is released or in progress.',
        ),
      })
    }
    if (!operation.isReportingPoint) {
      throw new CrudHttpError(422, {
        error: translate('manufacturing.errors.report_not_reporting_point', 'This operation is not a reporting point.'),
      })
    }
    if (operation.status === 'done') {
      throw new CrudHttpError(409, {
        error: translate(
          'manufacturing.errors.report_operation_already_done',
          'This operation has already been finalized by another report.',
        ),
      })
    }

    const allOperations = await em.find(ManufacturingOrderOperation, { orderId: order.id, deletedAt: null })
    ...
    let report!: ManufacturingReport
    ...
    await withAtomicFlush(
      ...
      [
        () => {
          report = em.create(ManufacturingReport, {
            tenantId,
            organizationId,
      ...
      { transaction: true, label: 'manufacturing.reports.create' },
    )
    ...
    await emitManufacturingEvent('manufacturing.report.created', { id: report.id, tenantId, organizationId })

    // Note: the order's released -> in_progress transition (first report)
    // does NOT re-emit `manufacturing.order.released` — that event already
    ...
    if (becameCompleted) {
      await emitManufacturingEvent('manufacturing.order.completed', { id: order.id, tenantId, organizationId })
    }
  ...
  async buildLog({ input, result }) {
    const { translate } = await resolveTranslations()
    return {
      actionLabel: translate('manufacturing.audit.report.create', 'Record manufacturing report'),
      resourceKind: 'manufacturing.report',
...
// ---------------------------------------------------------------------------
// manufacturing.reports.reverse — storno (compensating report)
// ---------------------------------------------------------------------------
...
const reverseReportCommand: CommandHandler<ReportReverseInput, ReportReverseResult> = {
  id: 'manufacturing.reports.reverse',
  isUndoable: false,
  ...
  async execute(input, ctx) {
    ...
    const original = await em.findOne(ManufacturingReport, { id: input.id, tenantId, organizationId })
    if (!original) {
      throw new CrudHttpError(404, { error: '[internal] Manufacturing report not found' })
    }
    if (original.reversesReportId) {
      throw new CrudHttpError(422, {
        error: translate(
          'manufacturing.errors.report_reverse_of_reversal_not_allowed',
          'A compensating (storno) report cannot itself be reversed.',
        ),
      })
    }

    const existingReversal = await em.findOne(ManufacturingReport, {
    ...
    if (existingReversal) {
      throw new CrudHttpError(409, {
        error: translate('manufacturing.errors.report_already_reversed', 'This report has already been reversed.'),
      })
    }
    ...
    let compensating!: ManufacturingReport
    try {
      ...
      await withAtomicFlush(
        ...
        [
          () => {
            compensating = em.create(ManufacturingReport, {
              tenantId,
              organizationId,
            ...
            // full rationale; `ManufacturingOrder` has no `completedAt`-style
        ...
        { transaction: true, label: 'manufacturing.reports.reverse' },
      )
    } catch (err) {
    ...
    await emitManufacturingEvent('manufacturing.report.reversed', { id: original.id, tenantId, organizationId })
  ...
  async buildLog({ input, result }) {
    const { translate } = await resolveTranslations()
    return {
      actionLabel: translate('manufacturing.audit.report.reverse', 'Reverse manufacturing report'),
      resourceKind: 'manufacturing.report',
      resourceId: result.id,
      relatedResourceKind: 'manufacturing.report',
