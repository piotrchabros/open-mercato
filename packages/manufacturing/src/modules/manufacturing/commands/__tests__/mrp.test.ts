...
const isManufacturingEnabledForTenantMock = jest.fn().mockResolvedValue(true)

jest.mock('../../lib/manufacturingToggle.js', () => ({
  isManufacturingEnabledForTenant: (...args: unknown[]) => isManufacturingEnabledForTenantMock(...args),
}))
...
function loadCommands(): Record<string, any> {
  ...
  for (const [fullId, cmd] of Object.entries(byFullId)) {
    const suffix = fullId.replace('manufacturing.mrp.', '')
    commands[suffix] = cmd
  }
  return commands
}

describe('manufacturing.mrp commands', () => {
  beforeEach(() => {
    registerCommand.mockClear()
    emitManufacturingEventMock.mockClear()
    emitCrudSideEffectsMock.mockClear()
    queueEnqueueMock.mockClear()
    isManufacturingEnabledForTenantMock.mockClear()
    isManufacturingEnabledForTenantMock.mockResolvedValue(true)
  })
  ...
  describe('cronFanOut', () => {
    it('enqueues one run per scope with planning params when manufacturing is enabled for that tenant', async () => {
      const { em } = makeMockEm()
      ...
      expect(isManufacturingEnabledForTenantMock).toHaveBeenCalledWith('tenant-enabled')
      expect(queueEnqueueMock).toHaveBeenCalledTimes(1)
    ...
    it('REGRESSION: skips a toggled-off tenant that still has planning-params rows (toggle off => no observable change, incl. cron)', async () => {
      ...
      isManufacturingEnabledForTenantMock.mockResolvedValue(false)
      const commands = loadCommands()
      const ctx = makeCtx(em, jest.fn())

      const result = await commands.cronFanOut.execute({}, ctx)

      expect(isManufacturingEnabledForTenantMock).toHaveBeenCalledWith('tenant-disabled')
  ...
  describe('acceptSuggestions', () => {
    it('accepting a MAKE suggestion creates a draft manufacturing order via commandBus and marks the suggestion accepted', async () => {
    ...
    it('accepting a BUY suggestion does not call commandBus but emits manufacturing.mrp_suggestion.accepted (purchasing seam)', async () => {
