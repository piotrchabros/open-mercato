...
describe('manufacturing.work_centers commands', () => {
  beforeEach(() => {
    jest.clearAllMocks()
    jest.resetModules()
  })
  ...
  it('create -> update -> delete round-trips through the mocked em', async () => {
    const { create, update, del } = (() => {
      ...
      return {
        create: cmds['manufacturing.work_centers.create'],
        update: cmds['manufacturing.work_centers.update'],
        del: cmds['manufacturing.work_centers.delete'],
      }
    })()
    ...
    // 'manufacturing.work_center.updated' event (not just created/deleted).
    ...
    for (const call of calls) {
      expect(call.indexer).toEqual({ entityType: E.manufacturing.work_center })
    }
    expect(calls[1].events).toEqual({ module: 'manufacturing', entity: 'work_center', persistent: true })
  })

  it('update undo restores the prior field values', async () => {
    const cmds = loadCommands()
    const update = cmds['manufacturing.work_centers.update']
    ...
    expect(calls.every((c: any) => c.indexer?.entityType === E.manufacturing.work_center)).toBe(true)
  })
})
...
describe('loadActiveBomGraph', () => {
  ...
  it('returns uomByComponentKey alongside the graph, from both active-BOM items and override items (task 1.4)', async () => {
    ...
    const { em, seed } = makeMockEm()
    const ManufacturingBom = { name: 'ManufacturingBom' }
    const ManufacturingBomItem = { name: 'ManufacturingBomItem' }

    // Active BOM for product-b: its item's uom (KG) should show up in the map.
    seed(ManufacturingBom, {
      id: 'bom-b',
      tenantId: 'tenant-1',
      organizationId: 'org-1',
      productId: 'product-b',
...
describe('manufacturing.boms.activate', () => {
  beforeEach(() => {
    jest.clearAllMocks()
    jest.resetModules()
  })
  ...
  it('rejects activation when the BOM graph would form a cycle', async () => {
    const cmds = loadCommands()
    const activate = cmds['manufacturing.boms.activate']
    ...
    const bomA = await em.findOne(ManufacturingBom, { id: 'bom-a' })
  ...
  it('activates a non-cyclic BOM and archives the previously active version', async () => {
    ...
    const newBom = await em.findOne(ManufacturingBom, { id: 'bom-new' })
    const oldBom = await em.findOne(ManufacturingBom, { id: 'bom-old' })
    ...
    // since 'manufacturing.bom.activated' is the declared lifecycle event).
    const calls = (ctx.__dataEngine.markOrmEntityChange as jest.Mock).mock.calls.map(([opts]) => opts)
    expect(calls).toHaveLength(2)
    expect(calls.every((c: any) => c.action === 'updated' && c.indexer?.entityType === E.manufacturing.manufacturing_bom && c.events === undefined)).toBe(true)
  ...
  it('ignores an active BOM under a different tenant with the same productId (spec Risks: cross-tenant leakage)', async () => {
    ...
    const tenant1Bom = await em.findOne(ManufacturingBom, { id: 'bom-tenant1' })
    const otherTenantBom = await em.findOne(ManufacturingBom, { id: 'bom-other-tenant' })
