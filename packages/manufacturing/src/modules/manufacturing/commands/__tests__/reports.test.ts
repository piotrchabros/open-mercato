...
function loadCommands(): Record<string, any> {
  ...
  for (const [fullId, cmd] of Object.entries(byFullId)) {
    commands[fullId.replace('manufacturing.reports.', '')] = cmd
  }
  return commands
}
...
function seedReleasedOrder(
  ...
  const order = seed({ name: 'ManufacturingOrder' }, {
  ...
  const operation = seed({ name: 'ManufacturingOrderOperation' }, {
...
describe('manufacturing.reports commands', () => {
  beforeEach(() => {
    jest.clearAllMocks()
    jest.resetModules()
  })
  ...
  it('a partial report increments operation quantities and transitions released -> in_progress on the first report', async () => {
    ...
    const operation = (await em.findOne({ name: 'ManufacturingOrderOperation' }, { id: operationId })) as Record<string, unknown>
    ...
    expect(emitManufacturingEventMock).toHaveBeenCalledWith('manufacturing.report.created', expect.objectContaining({ id: result.id }))
  })
  ...
  it('a final report on the LAST reporting-point operation calls provider.receive and completes the order', async () => {
    ...
    expect(emitManufacturingEventMock).toHaveBeenCalledWith('manufacturing.order.completed', expect.objectContaining({ id: orderId }))
  })
  ...
  it('a final report on a NON-last reporting-point operation does NOT trigger provider.receive or complete the order; only the LAST op final report does — receive is called exactly once with the correct final quantity (reviewer follow-up)', async () => {
    ...
    seed({ name: 'ManufacturingOrderOperation' }, {
    ...
    let order = (await em.findOne({ name: 'ManufacturingOrder' }, { id: orderId })) as Record<string, unknown>
    ...
    const opOne = (await em.findOne({ name: 'ManufacturingOrderOperation' }, { id: 'op-1' })) as Record<string, unknown>
    ...
    order = (await em.findOne({ name: 'ManufacturingOrder' }, { id: orderId })) as Record<string, unknown>
    expect(order.status).toBe('completed')
  ...
  describe('backflush matrix', () => {
    it('issues qtyPerUnit * reportedGoodQty in the stock item uom (passthrough, no conversion needed)', async () => {
      ...
      const material = (await em.findOne({ name: 'ManufacturingOrderMaterial' }, { id: 'mat-1' })) as Record<string, unknown>
  ...
  describe('storno (reverse)', () => {
    it('reverses the movements a report created and decrements operation/material quantities', async () => {
      ...
      expect(emitManufacturingEventMock).toHaveBeenCalledWith('manufacturing.report.reversed', expect.objectContaining({ id: created.id }))
    })
  ...
  describe('concurrency — two operators finalize the same operation', () => {
    it('the first finalize wins; the second (stale aggregate token) gets a translated 409 conflict', async () => {
      ...
      const orderBefore = (await em.findOne({ name: 'ManufacturingOrder' }, { id: 'order-1' })) as Record<string, unknown>
  ...
  it('registers both commands', () => {
    loadCommands()
    expect(registerCommand).toHaveBeenCalledWith(expect.objectContaining({ id: 'manufacturing.reports.create' }))
    expect(registerCommand).toHaveBeenCalledWith(expect.objectContaining({ id: 'manufacturing.reports.reverse' }))
  })
})
