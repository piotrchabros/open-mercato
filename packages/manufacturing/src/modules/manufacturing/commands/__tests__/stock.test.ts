...
// Mocked container harness for manufacturing stock commands, modeled on
// commands/__tests__/technology.test.ts. Unlike technology commands, stock
// commands do not touch `em` directly — they resolve `manufacturingStockProvider`
...
function makeCtx(provider: ReturnType<typeof makeProvider>, overrides: Record<string, unknown> = {}) {
  const resolve = jest.fn((key: string) => {
    if (key === 'manufacturingStockProvider') return provider
    return undefined
  })
  return {
...
describe('manufacturing stock commands', () => {
  it('registers all three commands as non-undoable (decision h: storno is the compensating-movement "undo", not a classic undo)', () => {
    ...
    expect(Object.keys(cmds).sort()).toEqual([
      'manufacturing.stock.adjust',
      'manufacturing.stock.receive',
      'manufacturing.stock.reverseMovement',
    ])
    expect(cmds['manufacturing.stock.receive'].isUndoable).toBe(false)
    expect(cmds['manufacturing.stock.receive'].undo).toBeUndefined()
    expect(cmds['manufacturing.stock.adjust'].isUndoable).toBe(false)
    expect(cmds['manufacturing.stock.adjust'].undo).toBeUndefined()
    expect(cmds['manufacturing.stock.reverseMovement'].isUndoable).toBe(false)
    expect(cmds['manufacturing.stock.reverseMovement'].undo).toBeUndefined()
  })

  describe('manufacturing.stock.receive', () => {
    it('delegates to the DI manufacturingStockProvider with tenant/org scope, sourceType manual', async () => {
      const provider = makeProvider()
      const ctx = makeCtx(provider)
      const { commands: cmds, errors } = loadCommands()

      const result = await cmds['manufacturing.stock.receive'].execute(
    ...
    it('buildLog records the movement id + tenant/org scope for the audit trail', async () => {
      ...
      const log = await cmds['manufacturing.stock.receive'].buildLog({ input, result, ctx, snapshots: {} })

      expect(log.resourceKind).toBe('manufacturing.stock_movement')
    ...
    it('maps StockUomMismatchError to a 422 CrudHttpError with a translated message, not the raw [internal] message', async () => {
      ...
      let caught: unknown
      try {
        await cmds['manufacturing.stock.receive'].execute({ productId: 'prod-1', variantId: null, qty: 10, uom: 'pcs' }, ctx)
      } catch (err) {
        caught = err
      }
  ...
  describe('manufacturing.stock.adjust', () => {
    ...
    it('buildLog carries the free-text reason in the audit context, not the movement payload column', async () => {
      ...
      const log = await cmds['manufacturing.stock.adjust'].buildLog({ input, result, ctx, snapshots: {} })
    ...
    it('maps InsufficientStockError to a 422 CrudHttpError with a translated message, not the raw [internal] message', async () => {
      ...
      let caught: unknown
      try {
        await cmds['manufacturing.stock.adjust'].execute(
  ...
  describe('manufacturing.stock.reverseMovement', () => {
    it('delegates to the provider (resolved via the same DI token) reverseMovement(movementId, scope)', async () => {
      const provider = makeProvider()
      const ctx = makeCtx(provider)
      const { commands: cmds, errors } = loadCommands()

      const result = await cmds['manufacturing.stock.reverseMovement'].execute({ movementId: 'mv-1' }, ctx)
    ...
    it('maps DoubleReversalError to a 409 CrudHttpError with a translated message, not the raw [internal] message (which embeds the movement UUID)', async () => {
      ...
      let caught: unknown
      try {
        await cmds['manufacturing.stock.reverseMovement'].execute({ movementId }, ctx)
      } catch (err) {
        caught = err
      }
      expect(isCrudHttpError(caught)).toBe(true)
    ...
    it('buildLog links back to the original movement via relatedResourceId', async () => {
      ...
      const log = await cmds['manufacturing.stock.reverseMovement'].buildLog({ input, result, ctx, snapshots: {} })
