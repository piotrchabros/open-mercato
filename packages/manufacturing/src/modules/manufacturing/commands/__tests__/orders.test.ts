...
// Mocked em/container harness for manufacturing order commands, modeled on
...
const emitManufacturingEventMock = jest.fn()

jest.mock('../../events.js', () => ({
  emitManufacturingEvent: (...args: unknown[]) => emitManufacturingEventMock(...args),
}))
...
function makeCtx(em: unknown, overrides: Record<string, unknown> = {}) {
  ...
  const resolve = jest.fn((key: string) => {
    ...
    if (key === 'manufacturingStockProvider') return stockProvider
 ...
 * short suffix after `manufacturing.orders.` (e.g. `create`, `plan`, `release`)
 * so call sites can write `cmds.release.execute(...)` instead of the full
 * `cmds['manufacturing.orders.release']`.
 */
function loadCommands(): Record<string, any> {
  ...
  for (const [fullId, cmd] of Object.entries(byFullId)) {
    const suffix = fullId.replace('manufacturing.orders.', '')
    commands[suffix] = cmd
  }
  return commands
}
...
function loadCommandsWithMikroOrmCore(): { commands: Record<string, any>; mikroOrmCore: typeof import('@mikro-orm/core') } {
  ...
  const commands: Record<string, any> = {}
  for (const [fullId, cmd] of Object.entries(byFullId)) {
    commands[fullId.replace('manufacturing.orders.', '')] = cmd
  }
...
describe('manufacturing.orders commands', () => {
  beforeEach(() => {
    jest.clearAllMocks()
    jest.resetModules()
  })
  ...
  it('create persists a draft order and emits created side effects', async () => {
    ...
    expect(callArgs[0]).toMatchObject({
      action: 'created',
      indexer: { entityType: E.manufacturing.manufacturing_order },
    })

    const order = (await em.findOne({ name: 'ManufacturingOrder' }, { id: result.id })) as Record<string, unknown>
  ...
  it('release copies active BOM items and routing operations as snapshot rows', async () => {
    ...
    seed({ name: 'ManufacturingBomItem' }, {
    ...
    const materials = await em.find({ name: 'ManufacturingOrderMaterial' }, { orderId })
    ...
    const operations = await em.find({ name: 'ManufacturingOrderOperation' }, { orderId })
    ...
    const order = (await em.findOne({ name: 'ManufacturingOrder' }, { id: orderId })) as Record<string, unknown>
  ...
  it('a BOM edit AFTER release does not affect the already-released order snapshot', async () => {
    ...
    const bomRow = seed({ name: 'ManufacturingBom' }, {
      id: 'bom-1', tenantId: SCOPE.tenantId, organizationId: SCOPE.organizationId,
      productId: 'product-1', variantId: null, version: 1, status: 'active', name: 'BOM v1', deletedAt: null,
    ...
    const materialsBefore = await em.find({ name: 'ManufacturingOrderMaterial' }, { orderId })
    ...
    const bomItemRow = (await em.findOne({ name: 'ManufacturingBomItem' }, { id: 'bom-item-1' })) as Record<string, unknown>
    bomItemRow.qtyPerUnit = '999'
    bomRow.name = 'BOM v1 (edited after release)'

    const materialsAfter = await em.find({ name: 'ManufacturingOrderMaterial' }, { orderId })
  ...
  it('release rejects with a translated 422 when no active BOM exists', async () => {
    ...
    // No active ManufacturingBom seeded.
  ...
  it('cancel is blocked with a translated 409 when a material was partially issued', async () => {
    ...
    if (materials.length === 0) {
      seed({ name: 'ManufacturingOrderMaterial' }, {
  ...
  it('emits the declared manufacturing.order.released and manufacturing.order.cancelled lifecycle events', async () => {
    const { em, seed } = makeMockEm()
    const ctx = makeCtx(em)
    const cmds = loadCommands()
    ...
    expect(ctx.__dataEngine.markOrmEntityChange).toHaveBeenCalled()
    expect(emitManufacturingEventMock).toHaveBeenCalledWith(
  ...
  describe('release-time material reservations + shortage list (task 3.2)', () => {
    ...
    /**
     * Same shape as {@link releaseWithBomItem}, but seeds N `ManufacturingBomItem`
     * rows (each becomes its own `ManufacturingOrderMaterial` snapshot line at
  ...
  it('create translates a unique-constraint number race into a translated 409 conflict', async () => {
    ...
    em.flush = jest.fn(async () => {
      throw new mikroOrmCore.UniqueConstraintViolationException(
        new Error('duplicate key value violates unique constraint "manufacturing_orders_scope_number_unique"'),
      )
    })
