...
// (b) emits the matching `manufacturing.<entity>.<action>` event declared in
// events.ts. `manufacturing.planning_params` has no declared CRUD events (only
// `manufacturing.mrp.*` lifecycle events exist for that resource), so it is
// indexer-only — no `CrudEventsConfig` is defined for it.
// ---------------------------------------------------------------------------

const workCenterCrudIndexer: CrudIndexerConfig<WorkCenter> = { entityType: E.manufacturing.work_center }
const workCenterCrudEvents: CrudEventsConfig<WorkCenter> = { module: 'manufacturing', entity: 'work_center', persistent: true }

const bomCrudIndexer: CrudIndexerConfig<ManufacturingBom> = { entityType: E.manufacturing.manufacturing_bom }
const bomCrudEvents: CrudEventsConfig<ManufacturingBom> = { module: 'manufacturing', entity: 'bom', persistent: true }

const routingCrudIndexer: CrudIndexerConfig<Routing> = { entityType: E.manufacturing.routing }
const routingCrudEvents: CrudEventsConfig<Routing> = { module: 'manufacturing', entity: 'routing', persistent: true }

const planningParamsCrudIndexer: CrudIndexerConfig<ProductPlanningParams> = { entityType: E.manufacturing.product_planning_params }
...
const createWorkCenterCommand: CommandHandler<WorkCenterCreateInput, { id: string }> = {
  ...
  async execute(input, ctx) {
    ...
    await withAtomicFlush(em, [() => { em.persist(wc) }], { transaction: true, label: 'manufacturing.work_centers.create' })
  ...
  async buildLog({ result, snapshots }) {
    ...
    return {
      actionLabel: translate('manufacturing.audit.work_center.create', 'Create work center'),
  ...
  async undo({ logEntry, ctx }) {
    ...
    if (wc) {
      await withAtomicFlush(em, [() => { em.remove(wc) }], { transaction: true, label: 'manufacturing.work_centers.create.undo' })
...
const updateWorkCenterCommand: CommandHandler<WorkCenterUpdateInput, { ok: boolean }> = {
  ...
  async execute(input, ctx) {
    ...
    await withAtomicFlush(
      ...
      { transaction: true, label: 'manufacturing.work_centers.update' },
    )
  ...
  async buildLog({ input, snapshots }) {
    ...
    const after = snapshots.after as WorkCenterSnapshot | undefined
    return {
      actionLabel: translate('manufacturing.audit.work_center.update', 'Update work center'),
      resourceKind: 'manufacturing.work_center',
      resourceId: input.id,
      tenantId: after?.tenantId ?? null,
      organizationId: after?.organizationId ?? null,
  ...
  async undo({ logEntry, ctx }) {
    ...
    if (wc) {
      await withAtomicFlush(
        ...
        { transaction: true, label: 'manufacturing.work_centers.update.undo' },
      )
      await emitCrudUndoSideEffects({
        dataEngine: resolveDataEngine(ctx),
        action: 'updated',
...
const deleteWorkCenterCommand: CommandHandler<{ id: string }, { ok: boolean }> = {
  ...
  async execute(input, ctx) {
    ...
    await withAtomicFlush(
      ...
      { transaction: true, label: 'manufacturing.work_centers.delete' },
    )
  ...
  async buildLog({ input, snapshots }) {
    ...
    const before = snapshots.before as WorkCenterSnapshot | undefined
    return {
      actionLabel: translate('manufacturing.audit.work_center.delete', 'Delete work center'),
      resourceKind: 'manufacturing.work_center',
      resourceId: input.id,
  ...
  async undo({ logEntry, ctx }) {
    ...
    if (wc) {
      await withAtomicFlush(
        ...
        { transaction: true, label: 'manufacturing.work_centers.delete.undo' },
      )
      await emitCrudUndoSideEffects({
        dataEngine: resolveDataEngine(ctx),
        action: 'created',
...
async function loadBomSnapshot(em: EntityManager, id: string): Promise<BomSnapshot | null> {
  const bom = await em.findOne(ManufacturingBom, { id })
  if (!bom) return null
  const items = await em.find(ManufacturingBomItem, { bomId: id, deletedAt: null })
  return {
    id: bom.id,
...
function syncBomItems(em: EntityManager, bomId: string, scope: { tenantId: string; organizationId: string }, items: Array<BomItemInputPayload | BomItemSnapshot>): void {
  ...
  for (const item of items) {
    em.create(ManufacturingBomItem, bomItemInputToEntityData(item as BomItemInputPayload, { ...scope, bomId }) as never)
  }
}

async function removeExistingBomItems(em: EntityManager, bomId: string): Promise<void> {
  const existing = await em.find(ManufacturingBomItem, { bomId })
...
async function nextBomVersion(em: EntityManager, scope: { tenantId: string; organizationId: string; productId: string; variantId: string | null }): Promise<number> {
  const rows = await em.find(ManufacturingBom, {
    tenantId: scope.tenantId,
    organizationId: scope.organizationId,
...
export async function loadActiveBomGraph(
  ...
  const activeBoms = await em.find(ManufacturingBom, {
    tenantId: scope.tenantId,
    organizationId: scope.organizationId,
    status: 'active',
    deletedAt: null,
  ...
  const items = activeBomIds.length ? await em.find(ManufacturingBomItem, { bomId: { $in: activeBomIds }, deletedAt: null }) : []
...
const createBomCommand: CommandHandler<BomCreateInput, { id: string }> = {
  ...
  async execute(input, ctx) {
    ...
    await withAtomicFlush(
      ...
      { transaction: true, label: 'manufacturing.boms.create' },
    )
  ...
  async buildLog({ result, snapshots }) {
    ...
    return {
      actionLabel: translate('manufacturing.audit.bom.create', 'Create BOM'),
  ...
  async undo({ logEntry, ctx }) {
    ...
    if (!after) return
    const em = ctx.container.resolve<EntityManager>('em').fork()
    const bom = await em.findOne(ManufacturingBom, { id: after.id })
    if (bom) {
      await withAtomicFlush(
        em,
        [
        ...
        { transaction: true, label: 'manufacturing.boms.create.undo' },
      )
      await emitCrudUndoSideEffects({
        dataEngine: resolveDataEngine(ctx),
        action: 'deleted',
...
const updateBomCommand: CommandHandler<BomUpdateInput, { ok: boolean }> = {
  ...
  async execute(input, ctx) {
    const em = ctx.container.resolve<EntityManager>('em').fork()
    const bom = await em.findOne(ManufacturingBom, { id: input.id, deletedAt: null })
    ...
    await withAtomicFlush(em, phases, { transaction: true, label: 'manufacturing.boms.update' })
  ...
  async buildLog({ input, snapshots }) {
    ...
    const after = snapshots.after as BomSnapshot | undefined
    return {
      actionLabel: translate('manufacturing.audit.bom.update', 'Update BOM'),
      resourceKind: 'manufacturing.bom',
      resourceId: input.id,
  ...
  async undo({ logEntry, ctx }) {
    ...
    if (!before) return
    const em = ctx.container.resolve<EntityManager>('em').fork()
    const bom = await em.findOne(ManufacturingBom, { id: before.id })
    if (!bom) return
    ...
    await withAtomicFlush(
      ...
      { transaction: true, label: 'manufacturing.boms.update.undo' },
    )
...
const deleteBomCommand: CommandHandler<{ id: string }, { ok: boolean }> = {
  ...
  async execute(input, ctx) {
    ...
    await withAtomicFlush(
      ...
      { transaction: true, label: 'manufacturing.boms.delete' },
    )
  ...
  async buildLog({ input, snapshots }) {
    ...
    const before = snapshots.before as BomSnapshot | undefined
    return {
      actionLabel: translate('manufacturing.audit.bom.delete', 'Delete BOM'),
      resourceKind: 'manufacturing.bom',
      resourceId: input.id,
      tenantId: before?.tenantId ?? null,
      organizationId: before?.organizationId ?? null,
  ...
  async undo({ logEntry, ctx }) {
    ...
    if (bom) {
      await withAtomicFlush(
        ...
        { transaction: true, label: 'manufacturing.boms.delete.undo' },
      )
      await emitCrudUndoSideEffects({
        dataEngine: resolveDataEngine(ctx),
        action: 'created',
...
const copyVersionBomCommand: CommandHandler<{ id: string }, { id: string }> = {
  id: 'manufacturing.boms.copyVersion',

  async execute(input, ctx) {
    const em = ctx.container.resolve<EntityManager>('em').fork()
    const source = await em.findOne(ManufacturingBom, { id: input.id, deletedAt: null })
    ...
    const sourceItems = await em.find(ManufacturingBomItem, { bomId: source.id, deletedAt: null })
    ...
    const copy = em.create(ManufacturingBom, {
      tenantId: source.tenantId,
      organizationId: source.organizationId,
      productId: source.productId,
      variantId: source.variantId ?? null,
    ...
    await withAtomicFlush(
      ...
      [
        ...
        () => {
          for (const item of sourceItems) {
            em.create(ManufacturingBomItem, {
              tenantId: source.tenantId,
              organizationId: source.organizationId,
              bomId: copy.id,
              componentProductId: item.componentProductId,
      ...
      { transaction: true, label: 'manufacturing.boms.copyVersion' },
    )
  ...
  async buildLog({ result, snapshots }) {
    ...
    const after = snapshots.after as BomSnapshot | undefined
    return {
      actionLabel: translate('manufacturing.audit.bom.copy_version', 'Copy BOM version'),
      resourceKind: 'manufacturing.bom',
      resourceId: result.id,
      tenantId: after?.tenantId ?? null,
      organizationId: after?.organizationId ?? null,
  ...
  async undo({ logEntry, ctx }) {
    ...
    if (bom) {
      await withAtomicFlush(
        ...
        { transaction: true, label: 'manufacturing.boms.copyVersion.undo' },
      )
      await emitCrudUndoSideEffects({
        dataEngine: resolveDataEngine(ctx),
        action: 'deleted',
...
const activateBomCommand: CommandHandler<{ id: string }, { ok: boolean; archivedSiblingIds: string[] }> = {
  id: 'manufacturing.boms.activate',
  ...
  async execute(input, ctx) {
    ...
    const items = await em.find(ManufacturingBomItem, { bomId: bom.id, deletedAt: null })
    ...
    if (cycle) {
      ...
      throw new CrudHttpError(422, {
        error: translate('manufacturing.errors.bom_cycle_detected', 'Activating this BOM would create a circular bill of materials.'),
        cycle,
      })
    }
    ...
    const otherActive = await em.find(ManufacturingBom, {
      tenantId: bom.tenantId,
      organizationId: bom.organizationId,
      productId: bom.productId,
      variantId: bom.variantId ?? null,
    ...
    await withAtomicFlush(
      ...
      { transaction: true, label: 'manufacturing.boms.activate' },
    )
    ...
    // `manufacturing.bom.activated` event below is the declared lifecycle event
    ...
    // redundant `manufacturing.bom.updated`.
    await emitCrudSideEffects({
      dataEngine,
      action: 'updated',
      entity: bom,
    ...
    await emitManufacturingEvent('manufacturing.bom.activated', { id: bom.id, tenantId: bom.tenantId, organizationId: bom.organizationId })
  ...
  async buildLog({ input, result, snapshots }) {
    ...
    const after = snapshots.after as BomSnapshot | undefined
    return {
      actionLabel: translate('manufacturing.audit.bom.activate', 'Activate BOM version'),
      resourceKind: 'manufacturing.bom',
      resourceId: input.id,
      tenantId: after?.tenantId ?? before?.tenantId ?? null,
      organizationId: after?.organizationId ?? before?.organizationId ?? null,
  ...
  async undo({ logEntry, ctx }) {
    ...
    const siblings = archivedSiblingIds.length
      ? await em.find(ManufacturingBom, { id: { $in: archivedSiblingIds }, deletedAt: null })
      : []
    ...
    await withAtomicFlush(
      ...
      { transaction: true, label: 'manufacturing.boms.activate.undo' },
    )
...
const createRoutingCommand: CommandHandler<RoutingCreateInput, { id: string }> = {
  id: 'manufacturing.routings.create',
  ...
  async execute(input, ctx) {
    ...
    await withAtomicFlush(
      ...
      { transaction: true, label: 'manufacturing.routings.create' },
    )
  ...
  async buildLog({ result, snapshots }) {
    ...
    return {
      actionLabel: translate('manufacturing.audit.routing.create', 'Create routing'),
  ...
  async undo({ logEntry, ctx }) {
    ...
    if (routing) {
      await withAtomicFlush(
        ...
        { transaction: true, label: 'manufacturing.routings.create.undo' },
      )
      await emitCrudUndoSideEffects({
        dataEngine: resolveDataEngine(ctx),
        action: 'deleted',
...
const updateRoutingCommand: CommandHandler<RoutingUpdateInput, { ok: boolean }> = {
  id: 'manufacturing.routings.update',
  ...
  async execute(input, ctx) {
    ...
    await withAtomicFlush(em, phases, { transaction: true, label: 'manufacturing.routings.update' })
  ...
  async buildLog({ input, snapshots }) {
    ...
    const after = snapshots.after as RoutingSnapshot | undefined
    return {
      actionLabel: translate('manufacturing.audit.routing.update', 'Update routing'),
      resourceKind: 'manufacturing.routing',
      resourceId: input.id,
      tenantId: after?.tenantId ?? before?.tenantId ?? null,
      organizationId: after?.organizationId ?? before?.organizationId ?? null,
  ...
  async undo({ logEntry, ctx }) {
    ...
    await withAtomicFlush(
      ...
      { transaction: true, label: 'manufacturing.routings.update.undo' },
    )
...
const deleteRoutingCommand: CommandHandler<{ id: string }, { ok: boolean }> = {
  id: 'manufacturing.routings.delete',
  ...
  async execute(input, ctx) {
    ...
    await withAtomicFlush(
      ...
      { transaction: true, label: 'manufacturing.routings.delete' },
    )
  ...
  async buildLog({ input, snapshots }) {
    ...
    const before = snapshots.before as RoutingSnapshot | undefined
    return {
      actionLabel: translate('manufacturing.audit.routing.delete', 'Delete routing'),
      resourceKind: 'manufacturing.routing',
      resourceId: input.id,
      tenantId: before?.tenantId ?? null,
      organizationId: before?.organizationId ?? null,
  ...
  async undo({ logEntry, ctx }) {
    ...
    if (routing) {
      await withAtomicFlush(
        ...
        { transaction: true, label: 'manufacturing.routings.delete.undo' },
      )
      await emitCrudUndoSideEffects({
        dataEngine: resolveDataEngine(ctx),
        action: 'created',
...
const copyVersionRoutingCommand: CommandHandler<{ id: string }, { id: string }> = {
  id: 'manufacturing.routings.copyVersion',
  ...
  async execute(input, ctx) {
    ...
    await withAtomicFlush(
      ...
      { transaction: true, label: 'manufacturing.routings.copyVersion' },
    )
  ...
  async buildLog({ result, snapshots }) {
    ...
    const after = snapshots.after as RoutingSnapshot | undefined
    return {
      actionLabel: translate('manufacturing.audit.routing.copy_version', 'Copy routing version'),
      resourceKind: 'manufacturing.routing',
      resourceId: result.id,
      tenantId: after?.tenantId ?? null,
      organizationId: after?.organizationId ?? null,
  ...
  async undo({ logEntry, ctx }) {
    ...
    if (routing) {
      await withAtomicFlush(
        em,
        [async () => removeExistingRoutingOperations(em, routing.id), () => { em.remove(routing) }],
        { transaction: true, label: 'manufacturing.routings.copyVersion.undo' },
      )
      await emitCrudUndoSideEffects({
        dataEngine: resolveDataEngine(ctx),
        action: 'deleted',
...
const activateRoutingCommand: CommandHandler<{ id: string }, { ok: boolean; archivedSiblingIds: string[] }> = {
  id: 'manufacturing.routings.activate',
  ...
  async execute(input, ctx) {
    ...
    await withAtomicFlush(
      ...
      { transaction: true, label: 'manufacturing.routings.activate' },
    )

    const dataEngine = resolveDataEngine(ctx)
    // Indexer-only (no `events`): `manufacturing.routing.activated` below is the
    // declared lifecycle event for this transition, so this call must not
    // also fire a redundant `manufacturing.routing.updated`.
    await emitCrudSideEffects({
      dataEngine,
      action: 'updated',
      entity: routing,
    ...
    await emitManufacturingEvent('manufacturing.routing.activated', { id: routing.id, tenantId: routing.tenantId, organizationId: routing.organizationId })
  ...
  async buildLog({ input, result, snapshots }) {
    ...
    const after = snapshots.after as RoutingSnapshot | undefined
    return {
      actionLabel: translate('manufacturing.audit.routing.activate', 'Activate routing version'),
      resourceKind: 'manufacturing.routing',
      resourceId: input.id,
      tenantId: after?.tenantId ?? before?.tenantId ?? null,
      organizationId: after?.organizationId ?? before?.organizationId ?? null,
  ...
  async undo({ logEntry, ctx }) {
    ...
    await withAtomicFlush(
      ...
      { transaction: true, label: 'manufacturing.routings.activate.undo' },
    )
...
const createPlanningParamsCommand: CommandHandler<PlanningParamsCreateInput, { id: string }> = {
  id: 'manufacturing.planning_params.create',
  ...
  async execute(input, ctx) {
    ...
    await withAtomicFlush(em, [() => { em.persist(row) }], { transaction: true, label: 'manufacturing.planning_params.create' })
  ...
  async buildLog({ result, snapshots }) {
    ...
    return {
      actionLabel: translate('manufacturing.audit.planning_params.create', 'Create planning parameters'),
      resourceKind: 'manufacturing.planning_params',
      resourceId: result.id,
      tenantId: after?.tenantId ?? null,
      organizationId: after?.organizationId ?? null,
      snapshotAfter: after,
  ...
  async undo({ logEntry, ctx }) {
    ...
    if (row) {
      await withAtomicFlush(em, [() => { em.remove(row) }], { transaction: true, label: 'manufacturing.planning_params.create.undo' })
      await emitCrudUndoSideEffects({
        dataEngine: resolveDataEngine(ctx),
        action: 'deleted',
        entity: row,
...
const updatePlanningParamsCommand: CommandHandler<PlanningParamsUpdateInput, { ok: boolean }> = {
  id: 'manufacturing.planning_params.update',
  ...
  async execute(input, ctx) {
    ...
    await withAtomicFlush(
      ...
      { transaction: true, label: 'manufacturing.planning_params.update' },
    )
  ...
  async buildLog({ input, snapshots }) {
    ...
    const after = snapshots.after as PlanningParamsSnapshot | undefined
    return {
      actionLabel: translate('manufacturing.audit.planning_params.update', 'Update planning parameters'),
      resourceKind: 'manufacturing.planning_params',
      resourceId: input.id,
      tenantId: after?.tenantId ?? before?.tenantId ?? null,
      organizationId: after?.organizationId ?? before?.organizationId ?? null,
  ...
  async undo({ logEntry, ctx }) {
    ...
    if (row) {
      await withAtomicFlush(
        ...
        { transaction: true, label: 'manufacturing.planning_params.update.undo' },
      )
      await emitCrudUndoSideEffects({
        dataEngine: resolveDataEngine(ctx),
        action: 'updated',
...
const deletePlanningParamsCommand: CommandHandler<{ id: string }, { ok: boolean }> = {
  id: 'manufacturing.planning_params.delete',
  ...
  async execute(input, ctx) {
    ...
    await withAtomicFlush(
      ...
      { transaction: true, label: 'manufacturing.planning_params.delete' },
    )
  ...
  async buildLog({ input, snapshots }) {
    ...
    const before = snapshots.before as PlanningParamsSnapshot | undefined
    return {
      actionLabel: translate('manufacturing.audit.planning_params.delete', 'Delete planning parameters'),
      resourceKind: 'manufacturing.planning_params',
      resourceId: input.id,
      tenantId: before?.tenantId ?? null,
      organizationId: before?.organizationId ?? null,
  ...
  async undo({ logEntry, ctx }) {
    ...
    if (row) {
      await withAtomicFlush(
        ...
        { transaction: true, label: 'manufacturing.planning_params.delete.undo' },
      )
      await emitCrudUndoSideEffects({
        dataEngine: resolveDataEngine(ctx),
        action: 'created',
