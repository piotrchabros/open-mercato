...
import {
  ...
  type ManufacturingStockProvider,
 ...
 * copies the currently-ACTIVE `ManufacturingBomItem`/`RoutingOperation` rows for
 * the order's product/variant into `ManufacturingOrderMaterial`/
 * `ManufacturingOrderOperation` as independent snapshot rows (decision g) inside
...
function resolveStockProvider(ctx: CommandRuntimeContext): ManufacturingStockProvider {
  return ctx.container.resolve<ManufacturingStockProvider>('manufacturingStockProvider')
}

const orderCrudIndexer: CrudIndexerConfig<ManufacturingOrder> = { entityType: E.manufacturing.manufacturing_order }
const orderCrudEvents: CrudEventsConfig<ManufacturingOrder> = { module: 'manufacturing', entity: 'order', persistent: true }
...
async function mapOrderTransitionError(err: unknown): Promise<unknown> {
  if (err instanceof IllegalOrderTransitionError) {
    ...
    return new CrudHttpError(422, {
      error: translate(
        'manufacturing.errors.order_illegal_transition',
        'This manufacturing order cannot move from "{from}" to "{to}".',
...
/**
 * Per-org sequence number for `ManufacturingOrder.number` (spec § Data Models —
 ...
 * `manufacturing_orders_scope_number_unique` constraint is the safety net
...
async function nextOrderNumber(em: EntityManager, scope: { tenantId: string; organizationId: string }): Promise<number> {
  const rows = await em.find(ManufacturingOrder, { tenantId: scope.tenantId, organizationId: scope.organizationId })
  return rows.reduce((max: number, row: ManufacturingOrder) => Math.max(max, row.number), 0) + 1
}

async function loadOrder(em: EntityManager, id: string): Promise<ManufacturingOrder> {
  const order = await em.findOne(ManufacturingOrder, { id, deletedAt: null })
  if (!order) throw new CrudHttpError(404, { error: '[internal] Manufacturing order not found' })
  return order
}
...
const createOrderCommand: CommandHandler<OrderCreateInput, { id: string }> = {
  ...
  async execute(input, ctx) {
    ...
    const order = em.create(ManufacturingOrder, {
      tenantId,
      organizationId,
      number,
    ...
    try {
      await withAtomicFlush(em, [() => { em.persist(order) }], { transaction: true, label: 'manufacturing.orders.create' })
    } catch (err) {
      ...
      // `manufacturing_orders_scope_number_unique` is the actual guard; surface
      ...
      if (err instanceof UniqueConstraintViolationException) {
        ...
        throw new CrudHttpError(409, {
          error: translate(
            'manufacturing.errors.order_number_conflict',
            'Another manufacturing order was just created with this number. Please try again.',
          ),
        })
      }
      throw err
  ...
  async buildLog({ input, result }) {
    const { translate } = await resolveTranslations()
    return {
      actionLabel: translate('manufacturing.audit.order.create', 'Create manufacturing order'),
      resourceKind: 'manufacturing.order',
...
// ---------------------------------------------------------------------------
// manufacturing.orders.update — header fields only, draft|planned only
// ---------------------------------------------------------------------------
...
const updateOrderCommand: CommandHandler<OrderUpdateInput, { ok: boolean }> = {
  ...
  async execute(input, ctx) {
    ...
    await enforceManufacturingOrderOptimisticLock(ctx, order)

    if (order.status !== 'draft' && order.status !== 'planned') {
      const { translate } = await resolveTranslations()
      throw new CrudHttpError(422, {
        error: translate(
          'manufacturing.errors.order_not_editable',
          'This manufacturing order can no longer be edited once released.',
        ),
      })
    }
    ...
    await withAtomicFlush(
      ...
      { transaction: true, label: 'manufacturing.orders.update' },
    )
  ...
  async buildLog({ input }) {
    const { translate } = await resolveTranslations()
    return {
      actionLabel: translate('manufacturing.audit.order.update', 'Update manufacturing order'),
      resourceKind: 'manufacturing.order',
      resourceId: input.id,
      payload: { input },
    }
...
// ---------------------------------------------------------------------------
// manufacturing.orders.delete — draft only
// ---------------------------------------------------------------------------

const deleteOrderCommand: CommandHandler<{ id: string }, { ok: boolean }> = {
  id: 'manufacturing.orders.delete',
  isUndoable: false,
  ...
  async execute(input, ctx) {
    ...
    if (order.status !== 'draft') {
      const { translate } = await resolveTranslations()
      throw new CrudHttpError(422, {
        error: translate('manufacturing.errors.order_delete_not_draft', 'Only draft manufacturing orders can be deleted.'),
      })
    }
    ...
    await withAtomicFlush(
      ...
      { transaction: true, label: 'manufacturing.orders.delete' },
    )
  ...
  async buildLog({ input }) {
    const { translate } = await resolveTranslations()
    return {
      actionLabel: translate('manufacturing.audit.order.delete', 'Delete manufacturing order'),
      resourceKind: 'manufacturing.order',
      resourceId: input.id,
    }
  },
...
// ---------------------------------------------------------------------------
// manufacturing.orders.plan — draft -> planned
// ---------------------------------------------------------------------------

const planOrderCommand: CommandHandler<{ id: string }, { ok: boolean }> = {
  id: 'manufacturing.orders.plan',
  isUndoable: false,
  ...
  async execute(input, ctx) {
    ...
    await withAtomicFlush(
      ...
      { transaction: true, label: 'manufacturing.orders.plan' },
    )
  ...
  async buildLog({ input }) {
    const { translate } = await resolveTranslations()
    return {
      actionLabel: translate('manufacturing.audit.order.plan', 'Plan manufacturing order'),
      resourceKind: 'manufacturing.order',
      resourceId: input.id,
    }
  },
...
// ---------------------------------------------------------------------------
// manufacturing.orders.release — planned -> released, snapshot copy (decision g)
// ---------------------------------------------------------------------------

/**
 * Reservation seam (spec § Status machine: "release ... emits reservations +
 * shortage list"). Wires the freshly-copied `ManufacturingOrderMaterial` rows
 * to `manufacturingStockProvider.reserve`, one line per material.
 *
...
const releaseOrderCommand: CommandHandler<{ id: string }, { ok: boolean; reservations: number; shortages: ShortageLine[] }> = {
  id: 'manufacturing.orders.release',
  isUndoable: false,
  ...
  async execute(input, ctx) {
    ...
    if (!bom) {
      ...
      throw new CrudHttpError(422, {
        error: translate(
          'manufacturing.errors.order_no_active_bom',
    ...
    if (!routing) {
      ...
      throw new CrudHttpError(422, {
        error: translate(
          'manufacturing.errors.order_no_active_routing',
          'This product has no active routing version to release against.',
        ),
      })
    }

    const bomItems = await em.find(ManufacturingBomItem, { bomId: bom.id, deletedAt: null })
    const routingOperations = await em.find(RoutingOperation, { routingId: routing.id, deletedAt: null })

    const createdMaterials: ManufacturingOrderMaterial[] = []
    ...
    await withAtomicFlush(
      ...
      [
        () => {
          for (const op of routingOperations) {
            const created = em.create(ManufacturingOrderOperation, {
              tenantId: order.tenantId,
              organizationId: order.organizationId,
              orderId: order.id,
          ...
          for (const item of bomItems) {
            const created = em.create(ManufacturingOrderMaterial, {
              tenantId: order.tenantId,
              organizationId: order.organizationId,
              orderId: order.id,
      ...
      { transaction: true, label: 'manufacturing.orders.release' },
    )
    ...
    await emitManufacturingEvent('manufacturing.order.released', {
      id: order.id,
      tenantId: order.tenantId,
      organizationId: order.organizationId,
    })
  ...
  async buildLog({ input }) {
    const { translate } = await resolveTranslations()
    return {
      actionLabel: translate('manufacturing.audit.order.release', 'Release manufacturing order'),
      resourceKind: 'manufacturing.order',
      resourceId: input.id,
    }
  },
...
// ---------------------------------------------------------------------------
// manufacturing.orders.cancel — draft|planned|released -> cancelled
// ---------------------------------------------------------------------------

const cancelOrderCommand: CommandHandler<{ id: string }, { ok: boolean }> = {
  id: 'manufacturing.orders.cancel',
  isUndoable: false,
  ...
  async execute(input, ctx) {
    ...
    const materials = await em.find(ManufacturingOrderMaterial, { orderId: order.id, deletedAt: null })
    const hasPartialIssue = materials.some((m: ManufacturingOrderMaterial) => Number(m.qtyIssued) > 0)
    if (hasPartialIssue) {
      const { translate } = await resolveTranslations()
      throw new CrudHttpError(409, {
        error: translate(
          'manufacturing.errors.order_cancel_blocked_partial_issue',
    ...
    await withAtomicFlush(
      ...
      { transaction: true, label: 'manufacturing.orders.cancel' },
    )
    ...
    await emitManufacturingEvent('manufacturing.order.cancelled', {
      id: order.id,
      tenantId: order.tenantId,
      organizationId: order.organizationId,
    })
  ...
  async buildLog({ input }) {
    const { translate } = await resolveTranslations()
    return {
      actionLabel: translate('manufacturing.audit.order.cancel', 'Cancel manufacturing order'),
      resourceKind: 'manufacturing.order',
      resourceId: input.id,
    }
  },
...
// ---------------------------------------------------------------------------
// manufacturing.orders.close — completed -> closed (terminal bookkeeping)
// ---------------------------------------------------------------------------

const closeOrderCommand: CommandHandler<{ id: string }, { ok: boolean }> = {
  id: 'manufacturing.orders.close',
  isUndoable: false,
  ...
  async execute(input, ctx) {
    ...
    await withAtomicFlush(
      ...
      { transaction: true, label: 'manufacturing.orders.close' },
    )
  ...
  async buildLog({ input }) {
    const { translate } = await resolveTranslations()
    return {
      actionLabel: translate('manufacturing.audit.order.close', 'Close manufacturing order'),
      resourceKind: 'manufacturing.order',
      resourceId: input.id,
    }
  },
...
export function transitionOrderToInProgress(order: ManufacturingOrder): void {
  assertOrderTransition(order.status, 'in_progress')
  order.status = 'in_progress'
  order.updatedAt = new Date()
}

export function transitionOrderToCompleted(order: ManufacturingOrder): void {
 ...
 */
export function reopenOrderFromReversal(order: ManufacturingOrder): void {
  order.status = 'in_progress'
  order.updatedAt = new Date()
}
