...
jest.mock('../../events.js', () => ({
  emitManufacturingEvent: jest.fn().mockResolvedValue(undefined),
}))
...
const emitManufacturingEventMock = emitManufacturingEvent as jest.Mock
...
describe('runMrpJob', () => {
  ...
  it('creates a ProgressJob, links it to the run, and reports progress through loading/computing/persisting', async () => {
    ...
    expect(emitManufacturingEventMock).toHaveBeenCalledWith(
      'manufacturing.mrp_run.completed',
