...
import { ManufacturingReport } from '../../../data/entities.js'
...
/**
 * Scrap-by-reason aggregation, task 6.1. Groups `ManufacturingReport.qtyScrap`
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const reports = await em.find(ManufacturingReport, where)
    ...
    const items = aggregates.map((bucket) => ({
      ...
      label:
        bucket.scrapReasonEntryId === UNSPECIFIED_SCRAP_REASON
          ? translate('manufacturing.analytics.scrap.unspecified', 'Unspecified')
    ...
    return NextResponse.json(
      { error: translate('manufacturing.errors.analytics_scrap_reasons_failed', 'Failed to load scrap-by-reason report.') },
      { status: 400 },
    )
  }
}
...
export const openApi: OpenApiRouteDoc = {
  tag: 'Manufacturing',
  summary: 'Manufacturing analytics — scrap by reason',
  methods: {
    GET: {
      operationId: 'getManufacturingScrapByReasonReport',
