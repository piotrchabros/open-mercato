 ...
 * `ManufacturingReport` aggregation query and the `DictionaryEntry` label
...
describe('GET /api/manufacturing/analytics/scrap-reasons scoping', () => {
  ...
  beforeEach(() => {
    ...
    findMock.mockImplementation(async (Entity: { name: string }) => {
      if (Entity.name === 'ManufacturingReport') {
        return [
  ...
  it('scopes both the report aggregation query and the dictionary-entry label lookup', async () => {
    const req = new NextRequest('http://localhost/api/manufacturing/analytics/scrap-reasons')
