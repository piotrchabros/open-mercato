 ...
 * `ManufacturingOrder`, `ManufacturingOrderMaterial`, and `ManufacturingOrderOperation`
...
describe('GET /api/manufacturing/analytics/consumption scoping', () => {
  ...
  beforeEach(() => {
    ...
    findMock.mockImplementation(async (Entity: { name: string }) => {
      if (Entity.name === 'ManufacturingOrder') {
  ...
  it('scopes the order, material, and operation queries by tenantId/organizationId', async () => {
    const req = new NextRequest('http://localhost/api/manufacturing/analytics/consumption')
  ...
  it('scales standardQty by (1 + scrapFactor) * consumedUnits using the operation-pinned cumulative qtyGood+qtyScrap (end-to-end wiring, review R1)', async () => {
    findMock.mockImplementation(async (Entity: { name: string }) => {
      if (Entity.name === 'ManufacturingOrder') return [{ id: 'order-1', number: 1 }]
      if (Entity.name === 'ManufacturingOrderMaterial') {
        return [
          {
            orderId: 'order-1',
      ...
      if (Entity.name === 'ManufacturingOrderOperation') {
