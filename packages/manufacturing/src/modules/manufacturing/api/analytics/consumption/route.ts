...
import { ManufacturingOrder, ManufacturingOrderMaterial, ManufacturingOrderOperation } from '../../../data/entities.js'
 ...
 * `ManufacturingOrderMaterial.qtyRequired` (material per ONE finished unit,
 ...
 * `ManufacturingOrderOperation` rows (loaded here in one batched, tenant/org
 ...
 * `planned` has no `ManufacturingOrderMaterial` rows yet.
 */
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const orders = await em.find(ManufacturingOrder, orderWhere)
    ...
    const materials = orderIds.length ? await em.find(ManufacturingOrderMaterial, materialWhere) : []
    ...
    const operations = orderIds.length ? await em.find(ManufacturingOrderOperation, operationWhere) : []
    ...
    return NextResponse.json(
      { error: translate('manufacturing.errors.analytics_consumption_failed', 'Failed to load consumption report.') },
      { status: 400 },
    )
  }
}
...
export const openApi: OpenApiRouteDoc = {
  tag: 'Manufacturing',
  summary: 'Manufacturing analytics — actual vs. standard consumption',
  methods: {
    GET: {
      operationId: 'getManufacturingConsumptionReport',
