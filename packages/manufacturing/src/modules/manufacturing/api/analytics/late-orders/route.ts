 ...
 * `api/reports/route.ts` already owns the shop-floor `ManufacturingReport`
 ...
 * Gated on `manufacturing.reports.view` (read-only oversight feature, already
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const orders = await em.find(ManufacturingOrder, {
    ...
    return NextResponse.json(
      { error: translate('manufacturing.errors.analytics_late_orders_failed', 'Failed to load late/at-risk orders.') },
      { status: 400 },
    )
  }
}
...
export const openApi: OpenApiRouteDoc = {
  tag: 'Manufacturing',
  summary: 'Manufacturing analytics — late/at-risk orders',
  methods: {
    GET: {
      operationId: 'listManufacturingLateAtRiskOrders',
      summary: 'List released/in-progress manufacturing orders that are late or at risk of being late',
      description:
