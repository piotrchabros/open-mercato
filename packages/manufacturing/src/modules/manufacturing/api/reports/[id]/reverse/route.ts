 ...
 * …/reports/[id]/reverse`). Gated by `manufacturing.reports.manage` (not
...
export const metadata = {
  requireAuth: true,
  requireFeatures: ['manufacturing.reports.manage'],
}
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> | { id: string } }) {
  ...
  try {
    ...
    const { result } = await commandBus.execute<{ id: string }, ReportReverseResult>('manufacturing.reports.reverse', {
      input: { id },
      ctx,
    })
    ...
    return NextResponse.json(
      { error: translate('manufacturing.errors.report_reverse_failed', 'Failed to reverse the manufacturing report.') },
      { status: 400 },
    )
  }
}
...
export const openApi: OpenApiRouteDoc = {
  tag: 'Manufacturing',
  summary: 'Reverse a manufacturing report',
  methods: {
    POST: {
      operationId: 'reverseManufacturingReport',
      summary: 'Storno a manufacturing report',
      description:
