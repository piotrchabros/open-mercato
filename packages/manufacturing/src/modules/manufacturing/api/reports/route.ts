...
import { ManufacturingReport, ManufacturingOrderOperation } from '../../data/entities.js'
...
/**
 * Report submission (spec § API Contracts: `POST /api/manufacturing/reports`).
 *
 ...
 * requires `manufacturing.operator.report` ONLY, not
 * `manufacturing.reports.manage`. Operators (spec decision e's minimal-feature
 * role) hold `manufacturing.operator.report` but NOT `manufacturing.reports.manage`
...
export const metadata = {
  GET: { requireAuth: true, requireFeatures: ['manufacturing.reports.view'] },
  POST: { requireAuth: true, requireFeatures: ['manufacturing.operator.report'] },
}
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // `ManufacturingReport` only stores `orderOperationId` (no direct `orderId`
    ...
    if (query.orderId) {
      const operations = await em.find(ManufacturingOrderOperation, {
    ...
    const [reports, total] = await em.findAndCount(ManufacturingReport, where, {
      limit: pageSize,
    ...
    return NextResponse.json(
      { error: translate('manufacturing.errors.reports_list_failed', 'Failed to load manufacturing reports.') },
      { status: 400 },
    )
  }
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { result } = await commandBus.execute<typeof input, ReportCreateResult>('manufacturing.reports.create', { input, ctx })
    ...
    return NextResponse.json(
      { error: translate('manufacturing.errors.report_create_failed', 'Failed to record the manufacturing report.') },
      { status: 400 },
    )
  }
}
...
export const openApi: OpenApiRouteDoc = {
  tag: 'Manufacturing',
  summary: 'Shop-floor manufacturing reports',
  methods: {
    GET: {
      operationId: 'listManufacturingReports',
      summary: 'List manufacturing reports for an operation',
    ...
    POST: {
      operationId: 'createManufacturingReport',
      summary: 'Record a partial or final shop-floor manufacturing report',
      description:
