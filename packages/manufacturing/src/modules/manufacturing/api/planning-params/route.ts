...
// (§ Access Control: `manufacturing.mrp.view/manage` -> "admin, Planista"), this
// route is gated by the MRP feature group rather than technology.manage.
// Recorded here for the follow-up spec delta (task 1.5).
const routeMetadata = {
  GET: { requireAuth: true, requireFeatures: ['manufacturing.mrp.view'] },
  POST: { requireAuth: true, requireFeatures: ['manufacturing.mrp.manage'] },
  PUT: { requireAuth: true, requireFeatures: ['manufacturing.mrp.manage'] },
  DELETE: { requireAuth: true, requireFeatures: ['manufacturing.mrp.manage'] },
}
...
const crud = makeCrudRoute({
  ...
  indexer: { entityType: E.manufacturing.product_planning_params },
  list: {
    entityId: E.manufacturing.product_planning_params,
  ...
  actions: {
    create: {
      commandId: 'manufacturing.planning_params.create',
    ...
    update: {
      commandId: 'manufacturing.planning_params.update',
      schema: rawBodySchema,
      mapInput: async ({ raw }) => planningParamsUpdateSchema.parse(raw),
      response: () => ({ ok: true }),
    },
    delete: {
      commandId: 'manufacturing.planning_params.delete',
      schema: rawBodySchema,
