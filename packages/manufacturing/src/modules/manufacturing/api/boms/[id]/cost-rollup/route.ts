...
import { ManufacturingBom, ManufacturingBomItem, Routing, RoutingOperation, WorkCenter } from '../../../../data/entities.js'
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> | { id: string } }) {
  ...
  try {
    ...
    const bom = await em.findOne(ManufacturingBom, { id, tenantId, ...orgScopeFilter, deletedAt: null })
    if (!bom) {
      return NextResponse.json({ error: translate('manufacturing.boms.error.not_found', 'BOM not found') }, { status: 404 })
    }

    const bomItems = await em.find(ManufacturingBomItem, { bomId: bom.id, ...orgScopeFilter, deletedAt: null })
    ...
    return NextResponse.json(
      { error: translate('manufacturing.boms.error.cost_rollup_failed', 'Failed to compute BOM cost rollup') },
      { status: 400 },
    )
  }
}
...
export const openApi: OpenApiRouteDoc = {
  ...
  methods: {
    GET: {
      operationId: 'getManufacturingBomCostRollup',
