...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> | { id: string } }) {
  ...
  try {
    ...
    const result = (await commandBus.execute('manufacturing.boms.copyVersion', { input: { id }, ctx })) as
    ...
    return NextResponse.json(
      { error: translate('manufacturing.errors.bom_copy_version_failed', 'Failed to copy BOM version.') },
      { status: 400 },
    )
  }
}
...
export const openApi: OpenApiRouteDoc = {
  ...
  methods: {
    POST: {
      operationId: 'copyManufacturingBomVersion',
      summary: 'Copy a BOM version',
