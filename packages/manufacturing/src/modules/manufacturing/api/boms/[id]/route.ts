...
import { ManufacturingBom, ManufacturingBomItem } from '../../../data/entities.js'
...
/**
 * Single-record BOM detail, including line items (the `manufacturing.boms` list
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> | { id: string } }) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: translate('manufacturing.boms.error.load_failed', 'Failed to load BOM') },
      { status: 400 },
    )
  }
}
