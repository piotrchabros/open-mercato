...
import { ManufacturingOrder, ManufacturingOrderOperation } from '../../../data/entities.js'
...
/**
 * Operator work queue (task 4.3, spec § API Contracts: `manufacturing.operator.view`).
...
export const metadata = {
  GET: { requireAuth: true, requireFeatures: ['manufacturing.operator.view'] },
}
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const orders = orderIds.length
      ? await em.find(ManufacturingOrder, {
          id: { $in: orderIds },
          tenantId,
          ...orgScopeFilter,
    ...
    return NextResponse.json(
      { error: translate('manufacturing.operator.error.queue_load_failed', 'Failed to load the operator work queue.') },
      { status: 400 },
    )
  }
}
...
export const openApi: OpenApiRouteDoc = {
  ...
  methods: {
    GET: {
      operationId: 'getManufacturingOperatorQueue',
