 ...
 * `manufacturing.operator.report` — the same feature the report-submission
...
export const metadata = {
  GET: { requireAuth: true, requireFeatures: ['manufacturing.operator.report'] },
}
  ...
  try {
    ...
    if (!dictionary) {
      return NextResponse.json(
        { error: translate('manufacturing.operator.error.scrap_reasons_missing', 'Scrap reason dictionary is not configured yet.') },
        { status: 404 },
      )
    }
    ...
    return NextResponse.json(
      { error: translate('manufacturing.operator.error.scrap_reasons_load_failed', 'Failed to load scrap reason dictionary.') },
      { status: 400 },
    )
  }
}
...
export const openApi: OpenApiRouteDoc = {
  ...
  methods: {
    GET: {
      operationId: 'getManufacturingScrapReasonDictionary',
      summary: 'Resolve the scrap-reason dictionary for the current organization scope',
      description: 'Returns the `manufacturing-scrap-reasons` dictionary entries used by the operator report form scrap-reason picker.',
      responses: [
