...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> | { id: string } }) {
  ...
  try {
    ...
    await commandBus.execute('manufacturing.routings.activate', { input: { id }, ctx })
    ...
    return NextResponse.json(
      { error: translate('manufacturing.errors.routing_activate_failed', 'Failed to activate routing version.') },
      { status: 400 },
    )
  }
}
...
export const openApi: OpenApiRouteDoc = {
  ...
  methods: {
    POST: {
      operationId: 'activateManufacturingRouting',
      summary: 'Activate a routing version',
      description:
