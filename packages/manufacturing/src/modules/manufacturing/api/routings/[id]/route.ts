...
/**
 * Single-record routing detail, including operations (the `manufacturing.routings`
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> | { id: string } }) {
  ...
  try {
    ...
    if (!routing) {
      return NextResponse.json({ error: translate('manufacturing.routings.error.not_found', 'Routing not found') }, { status: 404 })
    }
    ...
    return NextResponse.json(
      { error: translate('manufacturing.routings.error.load_failed', 'Failed to load routing') },
      { status: 400 },
    )
  }
}
...
export const openApi: OpenApiRouteDoc = {
  ...
  methods: {
    GET: {
      operationId: 'getManufacturingRouting',
