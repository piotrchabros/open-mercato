...
import { ManufacturingOrder } from '../../data/entities.js'
import {
  orderCreateSchema,
  orderUpdateSchema,
...
const routeMetadata = {
  GET: { requireAuth: true, requireFeatures: ['manufacturing.orders.view'] },
  POST: { requireAuth: true, requireFeatures: ['manufacturing.orders.manage'] },
  PUT: { requireAuth: true, requireFeatures: ['manufacturing.orders.manage'] },
  DELETE: { requireAuth: true, requireFeatures: ['manufacturing.orders.manage'] },
}
...
const crud = makeCrudRoute({
  ...
  list: {
    entityId: E.manufacturing.manufacturing_order,
  ...
  actions: {
    create: {
      commandId: 'manufacturing.orders.create',
    ...
    update: {
      commandId: 'manufacturing.orders.update',
      schema: rawBodySchema,
      mapInput: async ({ raw }) => orderUpdateSchema.parse(raw),
      response: () => ({ ok: true }),
    },
    delete: {
      commandId: 'manufacturing.orders.delete',
      schema: rawBodySchema,
...
export const openApi = createManufacturingCrudOpenApi({
  resourceName: 'ManufacturingOrder',
  pluralName: 'ManufacturingOrders',
  querySchema: orderListQuerySchema,
  listResponseSchema: createPagedListResponseSchema(orderListItemSchema),
  create: {
    schema: orderCreateSchema,
    description: 'Creates a new draft manufacturing order.',
  },
  update: {
    schema: orderUpdateSchema,
    responseSchema: defaultOkResponseSchema,
    description: 'Updates a draft/planned manufacturing order\'s header fields.',
  },
  del: {
    schema: z.object({ id: z.string().uuid() }),
    responseSchema: defaultOkResponseSchema,
    description: 'Soft-deletes a draft manufacturing order by ID.',
  },
})
