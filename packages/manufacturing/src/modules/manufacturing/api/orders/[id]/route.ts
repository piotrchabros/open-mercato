...
import { ManufacturingOrder, ManufacturingOrderOperation, ManufacturingOrderMaterial, MaterialReservation } from '../../../data/entities.js'
 ...
 * (the `manufacturing.orders` list endpoint is indexer-backed and only exposes
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> | { id: string } }) {
  ...
  try {
    ...
    if (!tenantId) {
      return NextResponse.json({ error: translate('manufacturing.errors.unauthorized', 'Unauthorized') }, { status: 401 })
    }
    ...
    const order = await em.findOne(ManufacturingOrder, {
      id,
      tenantId,
      ...orgScopeFilter,
      deletedAt: null,
    ...
    if (!order) {
      return NextResponse.json({ error: translate('manufacturing.orders.error.not_found', 'Manufacturing order not found') }, { status: 404 })
    }
    ...
    return NextResponse.json(
      { error: translate('manufacturing.orders.error.load_failed', 'Failed to load manufacturing order') },
      { status: 400 },
    )
  }
}
...
export const openApi: OpenApiRouteDoc = {
  tag: 'Manufacturing',
  summary: 'Get a manufacturing order by id (with operations and materials)',
  methods: {
    GET: {
      operationId: 'getManufacturingOrder',
      summary: 'Get a manufacturing order by id, including its released operations/materials snapshot',
      description: 'Returns the full order aggregate (header + operations + materials) for the detail UI.',
      responses: [
        { status: 200, description: 'Manufacturing order detail with operations and materials', schema: orderDetailSchema },
      ],
      errors: [
