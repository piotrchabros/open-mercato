...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> | { id: string } }) {
  ...
  try {
    ...
    await commandBus.execute('manufacturing.orders.plan', { input: { id }, ctx })
    ...
    return NextResponse.json(
      { error: translate('manufacturing.errors.order_plan_failed', 'Failed to plan the manufacturing order.') },
      { status: 400 },
    )
  }
}
...
export const openApi: OpenApiRouteDoc = {
  tag: 'Manufacturing',
  summary: 'Plan a manufacturing order',
  methods: {
    POST: {
      operationId: 'planManufacturingOrder',
      summary: 'Move a draft manufacturing order to planned',
      description: 'Transitions a manufacturing order from `draft` to `planned` (spec § Status machine).',
      responses: [
        { status: 200, description: 'Manufacturing order planned', schema: defaultOkResponseSchema },
      ],
      errors: [
