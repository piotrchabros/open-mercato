...
import { ManufacturingOrder, ManufacturingOrderMaterial } from '../../../../data/entities.js'
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> | { id: string } }) {
  ...
  try {
    ...
    const materials = await em.find(ManufacturingOrderMaterial, { orderId: order.id, ...orgScopeFilter, deletedAt: null })
    ...
    return NextResponse.json(
      { error: translate('manufacturing.errors.order_shortages_failed', 'Failed to load material shortages for this order.') },
      { status: 400 },
    )
  }
}
...
export const openApi: OpenApiRouteDoc = {
  tag: 'Manufacturing',
  summary: 'Get the current material shortage list for a manufacturing order',
  methods: {
    GET: {
      operationId: 'getManufacturingOrderShortages',
      summary: 'Recompute the material shortage list for a released manufacturing order',
      description:
