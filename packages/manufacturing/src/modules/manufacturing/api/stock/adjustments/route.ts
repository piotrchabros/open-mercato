...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { result } = await commandBus.execute<typeof input, { movementId: string }>('manufacturing.stock.adjust', { input, ctx })
    ...
    return NextResponse.json(
      { error: translate('manufacturing.errors.stock_adjust_failed', 'Failed to adjust stock.') },
      { status: 400 },
    )
  }
}
...
export const openApi: OpenApiRouteDoc = {
  tag: 'Manufacturing',
  summary: 'Adjust manufacturing stock',
  methods: {
    POST: {
      operationId: 'adjustManufacturingStock',
