 ...
 * `IMPORT_BATCH_SIZE` rows is received via its own `manufacturing.stock.receive`
 ...
 *
 * Each valid row goes through `commandBus.execute('manufacturing.stock.receive', ...)`
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!req.body) {
      return NextResponse.json({ error: translate('manufacturing.errors.stock_import_empty_body', 'Request body is empty') }, { status: 400 })
    }
    ...
    return NextResponse.json(
      { error: translate('manufacturing.errors.stock_import_failed', 'Failed to import stock CSV.') },
      { status: 400 },
    )
  }
    ...
    return NextResponse.json(
      {
        ...summary,
        error: translate(
          'manufacturing.errors.import_row_cap',
...
export const openApi: OpenApiRouteDoc = {
  tag: 'Manufacturing',
  summary: 'Import manufacturing stock from CSV',
  methods: {
    POST: {
      operationId: 'importManufacturingStock',
      summary: 'Stream-import stock receipts from a CSV file',
      description: `Streams a CSV body (columns: product_id, variant_id?, qty, uom, batch_number?, expires_at?), receiving each valid row via manufacturing.stock.receive. Above ${STOCK_IMPORT_MAX_ROWS} rows, returns 413 but the body still carries the PARTIAL {imported, failed} counts from batches that already ran before the cap tripped (capExceeded: true) — never a bare error with no counts.`,
