...
export const metadata = {
  requireAuth: true,
  requireFeatures: ['manufacturing.stock.manage'],
}
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> | { id: string } }) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: translate('manufacturing.errors.stock_reverse_failed', 'Failed to reverse stock movement.') },
      { status: 400 },
    )
  }
}
...
export const openApi: OpenApiRouteDoc = {
  tag: 'Manufacturing',
  summary: 'Reverse a manufacturing stock movement',
  methods: {
    POST: {
      operationId: 'reverseManufacturingStockMovement',
