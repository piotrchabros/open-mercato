...
/**
 * Client-safe id — importing the server-only `manufacturingToggle.ts` resolver
 * here would pull `createRequestContainer` into the browser bundle.
 */
const PRODUCTION_TOGGLE_ID = 'manufacturing_enabled'
...
async function checkManufacturingToggleEnabled(): Promise<boolean> {
...
function CreateOrderDialog({
  ...
  const { runMutation } = useGuardedMutation<{ orderId: string; lineId: string }>({
    contextId: `manufacturing:order-manufacturing-tab:${orderId}`,
  })
  ...
  const handleSubmit = React.useCallback(async () => {
    ...
    try {
      await runMutation({
        ...
        operation: async () => createCrud('manufacturing/orders', {
      ...
      setError(t('manufacturing.injection.orderTab.dialog.error', 'Failed to create manufacturing order.'))
    } finally {
      setSaving(false)
    }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        <DialogHeader>
          <DialogTitle>{t('manufacturing.injection.orderTab.dialog.title', 'Create manufacturing order')}</DialogTitle>
        </DialogHeader>
        <div className="space-y-3">
          <div className="space-y-1.5">
            <Label htmlFor="manufacturing-order-qty">{t('manufacturing.injection.orderTab.dialog.qty', 'Quantity')}</Label>
            <Input
              id="manufacturing-order-qty"
              type="number"
              value={qty}
          ...
          <div className="space-y-1.5">
            <Label htmlFor="manufacturing-order-uom">{t('manufacturing.injection.orderTab.dialog.uom', 'Unit')}</Label>
            <Input
              id="manufacturing-order-uom"
              value={uom}
              onChange={(event) => setUom(event.target.value)}
            />
          </div>
          <div className="space-y-1.5">
            <Label htmlFor="manufacturing-order-due-date">{t('manufacturing.injection.orderTab.dialog.dueDate', 'Due date')}</Label>
            <Input
              id="manufacturing-order-due-date"
              type="date"
              value={dueDate}
        ...
        <DialogFooter>
          <Button variant="outline" onClick={() => onOpenChange(false)}>
            {t('manufacturing.injection.orderTab.dialog.cancel', 'Cancel')}
          </Button>
          <Button onClick={() => void handleSubmit()} disabled={saving}>
            {t('manufacturing.injection.orderTab.dialog.submit', 'Create')}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
...
export const OrderManufacturingTabWidget: React.FC<InjectionWidgetComponentProps<unknown, unknown>> = ({ context }) => {
  const t = useT()
  const [toggleChecked, setToggleChecked] = React.useState(false)
  const [toggleEnabled, setToggleEnabled] = React.useState(false)
  const [loading, setLoading] = React.useState(true)
  const [error, setError] = React.useState<string | null>(null)
  const [orders, setOrders] = React.useState<ManufacturingOrderRow[]>([])
  ...
  React.useEffect(() => {
    let cancelled = false
    checkManufacturingToggleEnabled()
      .then((enabled) => {
        if (cancelled) return
  ...
  React.useEffect(() => {
    ...
    const load = async () => {
      const ordersList = await fetchCrudList<ManufacturingOrderRow>('manufacturing/orders', {
        sourceType: 'sales_order',
        sourceId: orderId,
        page: 1,
        pageSize: 100,
      }).catch(() => ({ items: [] as ManufacturingOrderRow[], total: 0, page: 1, pageSize: 100, totalPages: 0 }))
      ...
      await Promise.all(productIds.map(async (productId) => {
        const res = await apiCall<{ items?: Array<Record<string, unknown>> }>(
          `/api/manufacturing/planning-params?productId=${productId}&page=1&pageSize=1`,
          undefined,
          { fallback: { items: [] } },
        )
    ...
    load().catch(() => {
      if (cancelled) return
      setError(t('manufacturing.injection.orderTab.error', 'Failed to load manufacturing data.'))
      setLoading(false)
    })
  ...
  if (!toggleEnabled) {
    return (
      <div className="text-muted-foreground text-sm py-6 text-center">
        {t('manufacturing.injection.orderTab.disabled', 'The manufacturing module is not enabled for this tenant.')}
      </div>
    )
  }
      ...
      {loading ? (
        ...
        <>
          <div className="space-y-2">
            <div className="text-sm font-medium">{t('manufacturing.injection.orderTab.ordersTitle', 'Manufacturing orders')}</div>
            {orders.length ? (
              <div className="space-y-1.5">
                {orders.map((order) => (
              ...
              <div className="text-muted-foreground text-sm">
                {t('manufacturing.injection.orderTab.ordersEmpty', 'No manufacturing orders linked to this sales order yet.')}
              </div>
            )}
          </div>
          ...
          {makeLines.length ? (
            <div className="space-y-2">
              <div className="text-sm font-medium">{t('manufacturing.injection.orderTab.makeLinesTitle', 'Make-flagged lines')}</div>
              <div className="space-y-1.5">
                {makeLines.map((line) => (
                  <div key={line.id} className="flex items-center justify-between rounded-md border p-2 text-sm">
                    ...
                    <Button size="sm" variant="outline" onClick={() => setDialogLine(line)}>
                      {t('manufacturing.injection.orderTab.createButton', 'Create manufacturing order')}
                    </Button>
                  </div>
                ))}
              </div>
...
export default OrderManufacturingTabWidget
