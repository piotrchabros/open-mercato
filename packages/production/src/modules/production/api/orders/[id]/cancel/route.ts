...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> | { id: string } }) {
  ...
  try {
    ...
    await commandBus.execute('production.orders.cancel', { input: { id }, ctx })
    ...
    return NextResponse.json(
      { error: translate('production.errors.order_cancel_failed', 'Failed to cancel the production order.') },
      { status: 400 },
    )
  }
}
...
export const openApi: OpenApiRouteDoc = {
  ...
  methods: {
    POST: {
      ...
      summary: 'Cancel a draft, planned, or released production order',
      description:
      ...
      responses: [
        { status: 200, description: 'Production order cancelled', schema: defaultOkResponseSchema },
      ],
      errors: [
