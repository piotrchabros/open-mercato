...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> | { id: string } }) {
  ...
  try {
    ...
    await commandBus.execute('production.orders.plan', { input: { id }, ctx })
    ...
    return NextResponse.json(
      { error: translate('production.errors.order_plan_failed', 'Failed to plan the production order.') },
      { status: 400 },
    )
  }
}
...
export const openApi: OpenApiRouteDoc = {
  ...
  methods: {
    POST: {
      ...
      summary: 'Move a draft production order to planned',
      description: 'Transitions a production order from `draft` to `planned` (spec § Status machine).',
      responses: [
        { status: 200, description: 'Production order planned', schema: defaultOkResponseSchema },
      ],
      errors: [
