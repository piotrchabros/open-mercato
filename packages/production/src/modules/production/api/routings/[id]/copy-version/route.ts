...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> | { id: string } }) {
  ...
  try {
    ...
    const result = (await commandBus.execute('production.routings.copyVersion', { input: { id }, ctx })) as
    ...
    return NextResponse.json(
      { error: translate('production.errors.routing_copy_version_failed', 'Failed to copy routing version.') },
      { status: 400 },
    )
  }
}
...
export const openApi: OpenApiRouteDoc = {
  ...
  methods: {
    POST: {
      operationId: 'copyProductionRoutingVersion',
      summary: 'Copy a routing version',
