...
// The stored entry stays host-agnostic because `cacheKey` has no host component;
// the host-bound view is layered on at every read, so responses carry one extra field.
function expectedResponsePayload(overrides: Record<string, unknown> = {}) {
  return expectedPayload({ hostBoundOrganizationId: null, ...overrides })
}
...
describe('GET /api/directory/organization-switcher caching', () => {
  ...
  it('serves the second identical request from cache without repeating organization or RBAC work and still audits both reads', async () => {
    ...
    await expect(secondResponse.json()).resolves.toEqual(expectedResponsePayload())
