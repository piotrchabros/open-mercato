...
flowchart TD
  ...
  subgraph Primitives["Purge primitives"]
    ...
    all["all → `cache.clear()`"]
    structural["structural → `nav:*` + admin/portal nav segments"]
  end
...
- operators can still run `yarn mercato configs cache structural --all-tenants` for the full DI-aware path, including custom cache-service overrides
