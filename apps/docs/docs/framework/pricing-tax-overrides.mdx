...
- Keep the `.proxy()` — the request container runs in Awilix CLASSIC injection mode, where a destructured factory parameter silently receives `undefined` dependencies. See [Container & DI](/framework/ioc/container#injection-mode-classic).
