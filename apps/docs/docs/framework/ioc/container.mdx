...
## Declaring factory dependencies

The request container is built with `InjectionMode.CLASSIC`. Awilix reads the factory's **parameter names** and passes one resolved dependency per parameter, positionally — it never hands the factory a cradle object. So an `asFunction` factory must either list its dependencies as individually named parameters, or opt into proxy injection per registration:

```ts
// Correct — one parameter per DI key.
salesOrderService: asFunction((em: EntityManager, eventBus: EventBus) =>
  createSalesOrderService(em, eventBus),
).scoped(),

// Also correct — `.proxy()` switches this registration to cradle injection.
salesOrderService: asFunction(({ em, eventBus }: Cradle) =>
  createSalesOrderService(em, eventBus),
).scoped().proxy(),
```

Three shapes break under classic injection, and unit tests that inject fakes directly never see it:

| Shape | What happens |
| --- | --- |
| `asFunction(({ em }) => …)` without `.proxy()` | `em` is resolved positionally and handed in as the object being destructured, so every binding is `undefined`. Fails **silently** — the service degrades to "no data" instead of erroring. |
| `asFunction((cradle) => …)` without `.proxy()` | Awilix looks for a registration literally named `cradle` and throws `Could not resolve 'cradle'`. |
| `asFunction((em, cache?: Cache \| null) => …)` | TypeScript erases the `?` at transpile, so Awilix sees a required parameter and throws when `cache` is not registered. Write `cache: Cache \| null = null` — a runtime default is what marks a parameter optional. |

`packages/core/src/__tests__/di-classic-proxy.test.ts` enforces all three rules across every `asFunction` registration in `packages/` and `apps/`.
